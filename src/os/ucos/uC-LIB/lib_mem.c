...
CPU_INT08U Mem_Heap
	[LIB_MEM_CFG_HEAP_SIZE]; /* Mem heap.                                    */
#endif
#endif
...
#if (LIB_MEM_CFG_ALLOC_EN ==                                                   \
     DEF_ENABLED) /* -------------- MEM POOL FNCTS -------------- */

#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
static CPU_BOOLEAN Mem_PoolBlkIsValidAddr(MEM_POOL *pmem_pool, void *pmem_blk);
#endif

static CPU_SIZE_T Mem_SegCalcTotSize(void *pmem_addr, MEM_POOL_BLK_QTY blk_nbr,
				     CPU_SIZE_T blk_size, CPU_SIZE_T blk_align);

static void *Mem_SegAlloc(MEM_POOL *pmem_pool, CPU_SIZE_T size,
			  CPU_SIZE_T align);
...
void Mem_Init(void)
{
	...
	pmem_pool->SegHeadPtr =
		(MEM_POOL *)&Mem_PoolHeap; /* Heap seg head = heap seg.                    */
...
void Mem_Set(void *pmem, CPU_INT08U data_val, CPU_SIZE_T size)
{
	...
	data_align = 0u;
	for (i = 0u; i < sizeof(CPU_ALIGN);
	     i++) { /* Fill each data_align octet with data val.            */
		data_align <<= DEF_OCTET_NBR_BITS;
		data_align |= (CPU_ALIGN)data_val;
	}

	size_rem = size;
	mem_align_mod = (CPU_INT08U)(
		(CPU_ADDR)pmem %
		sizeof(CPU_ALIGN)); /* See Note #3.                                 */

	pmem_08 = (CPU_INT08U *)pmem;
	if (mem_align_mod !=
	    0u) { /* If leading octets avail,                   ...       */
		i = mem_align_mod;
		while ((size_rem >
			0) && /* ... start mem buf fill with leading octets ...       */
		       (i <
			sizeof(CPU_ALIGN))) { /* ... until next CPU_ALIGN word boundary.              */
			*pmem_08++ = data_val;
	...
	pmem_align = (CPU_ALIGN *)
		pmem_08; /* See Note #2a.                                        */
	while (size_rem >=
	       sizeof(CPU_ALIGN)) { /* While mem buf aligned on CPU_ALIGN word boundaries,  */
		*pmem_align++ =
			data_align; /* ... fill mem buf with    CPU_ALIGN-sized data.       */
		size_rem -= sizeof(CPU_ALIGN);
	}

	pmem_08 = (CPU_INT08U *)pmem_align;
	while (size_rem >
	       0) { /* Finish mem buf fill with trailing octets.            */
		*pmem_08++ = data_val;
		size_rem -= sizeof(CPU_INT08U);
	}
}
...
void Mem_Copy(void *pdest, const void *psrc, CPU_SIZE_T size)
{
	...
	if (mem_gap_octets >=
	    sizeof(CPU_ALIGN)) { /* Avoid bufs overlap.                                  */
		/* See Note #4.                                         */
		mem_align_mod_dest = (CPU_INT08U)((CPU_ADDR)pmem_08_dest %
						  sizeof(CPU_ALIGN));
		mem_align_mod_src =
			(CPU_INT08U)((CPU_ADDR)pmem_08_src % sizeof(CPU_ALIGN));

		mem_aligned = (mem_align_mod_dest == mem_align_mod_src) ?
				      DEF_YES :
				      DEF_NO;

		if (mem_aligned ==
		    DEF_YES) { /* If mem bufs' alignment offset equal, ...             */
			/* ... optimize copy for mem buf alignment.             */
			if (mem_align_mod_dest !=
			    0u) { /* If leading octets avail,                   ...       */
				i = mem_align_mod_dest;
				while ((size_rem >
					0) && /* ... start mem buf copy with leading octets ...       */
				       (i <
					sizeof(CPU_ALIGN))) { /* ... until next CPU_ALIGN word boundary.              */
			...
			pmem_align_dest = (CPU_ALIGN *)
				pmem_08_dest; /* See Note #3a.                                        */
			pmem_align_src = (const CPU_ALIGN *)pmem_08_src;
			while (size_rem >=
			       sizeof(CPU_ALIGN)) { /* While mem bufs aligned on CPU_ALIGN word boundaries, */
				*pmem_align_dest++ =
					*pmem_align_src++; /* ... copy psrc to pdest with CPU_ALIGN-sized words.   */
				size_rem -= sizeof(CPU_ALIGN);
			}
	...
	while (size_rem >
	       0) { /* For unaligned mem bufs or trailing octets, ...       */
		*pmem_08_dest++ =
			*pmem_08_src++; /* ... copy psrc to pdest by octets.                    */
		size_rem -= sizeof(CPU_INT08U);
	}
}
#endif
...
void Mem_Move(void *pdest, const void *psrc, CPU_SIZE_T size)
{
	CPU_SIZE_T size_rem;
	CPU_SIZE_T mem_gap_octets;
	CPU_ALIGN *pmem_align_dest;
	...
	if (mem_gap_octets >=
		...
		if (mem_aligned ==
			...
			if (mem_align_mod_dest !=
			    (sizeof(CPU_ALIGN) -
			     1)) { /* If leading octets avail,                   ...       */
				i = mem_align_mod_dest;
				while ((size_rem >
					0) && /* ... start mem buf copy with leading octets ...       */
				       (i >=
					0)) { /* ... until next CPU_ALIGN word boundary.              */
					*pmem_08_dest-- = *pmem_08_src--;
					size_rem -= sizeof(CPU_INT08U);
					i--;
				}
			...
			/* See Note #3a.                                        */
			pmem_align_dest =
				(CPU_ALIGN *)((CPU_INT08U *)pmem_08_dest -
					      sizeof(CPU_ALIGN) + 1);
			pmem_align_src =
				(const CPU_ALIGN *)((CPU_INT08U *)pmem_08_src -
						    sizeof(CPU_ALIGN) + 1);
			while (size_rem >=
			       sizeof(CPU_ALIGN)) { /* While mem bufs aligned on CPU_ALIGN word boundaries, */
				*pmem_align_dest-- =
...
CPU_BOOLEAN Mem_Cmp(const void *p1_mem, const void *p2_mem, CPU_SIZE_T size)
{
	...
	mem_cmp =
		DEF_YES; /* Assume mem bufs are identical until cmp fails.       */
	size_rem = size;
	...
	if (mem_aligned ==
		...
		if (mem_align_mod_1 !=
		    0u) { /* If trailing octets avail,                  ...       */
			i = mem_align_mod_1;
			while ((mem_cmp ==
				DEF_YES) && /* ... cmp mem bufs while identical &         ...       */
			       (size_rem >
				0) && /* ... start mem buf cmp with trailing octets ...       */
			       (i >
				0)) { /* ... until next CPU_ALIGN word boundary.              */
				p1_mem_08--;
				p2_mem_08--;
				if (*p1_mem_08 !=
				    *p2_mem_08) { /* If ANY data octet(s) NOT identical, cmp fails.       */
					mem_cmp = DEF_NO;
				}
				size_rem -= sizeof(CPU_INT08U);
				i--;
		...
		if (mem_cmp ==
		    DEF_YES) { /* If cmp still identical, cmp aligned mem bufs.        */
			p1_mem_align = (CPU_ALIGN *)
				p1_mem_08; /* See Note #3a.                                        */
			p2_mem_align = (CPU_ALIGN *)p2_mem_08;

			while ((mem_cmp ==
				DEF_YES) && /* Cmp mem bufs while identical & ...                   */
			       (size_rem >=
				sizeof(CPU_ALIGN))) { /* ... mem bufs aligned on CPU_ALIGN word boundaries.   */
				p1_mem_align--;
				p2_mem_align--;
				if (*p1_mem_align !=
				    *p2_mem_align) { /* If ANY data octet(s) NOT identical, cmp fails.       */
					mem_cmp = DEF_NO;
				}
				size_rem -= sizeof(CPU_ALIGN);
			}
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
void *Mem_HeapAlloc(CPU_SIZE_T size, CPU_SIZE_T align, CPU_SIZE_T *poctets_reqd,
		    LIB_ERR *perr)
{
	...
	CPU_SR_ALLOC();
     ...
     DEF_ENABLED) /* ------------- VALIDATE RTN ERR PTR ------------- */
	if (perr == (LIB_ERR *)0) {
	...
	if (poctets_reqd ==
		...
		poctets_reqd =
			(CPU_SIZE_T
				 *)&octets_reqd_unused; /* ... re-cfg NULL rtn ptr to unused local var.     */
		(void)&octets_reqd_unused; /* Prevent possible 'variable unused' warning.      */
	}
	*poctets_reqd =
		0u; /* Init octets req'd for err (see Note #1).         */

#if (LIB_MEM_CFG_ARG_CHK_EXT_EN ==                                             \
     DEF_ENABLED) /* ------------ VALIDATE HEAP MEM ALLOC ----------- */
	if (size < 1) {
	...
	if (size_req <
		...
		CPU_CRITICAL_EXIT();
		*poctets_reqd =
			size; /* ... rtn add'l heap size needed.                  */
	...
	if (size_req >
	    size_rem) { /* If req'd size > rem heap size, ...               */
		CPU_CRITICAL_EXIT();
		*poctets_reqd =
			size_req -
			size_rem; /* ... rtn add'l heap size needed.                  */
	...
	if (pmem_blk ==
	    (void *)0) { /* If mem blk NOT avail from heap, ...              */
		CPU_CRITICAL_EXIT();
		*poctets_reqd =
			size_req; /* ... rtn add'l heap size needed.                  */
		*perr = LIB_MEM_ERR_HEAP_EMPTY;
		return ((void *)0);
	}
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
CPU_SIZE_T Mem_SegGetSizeRem(MEM_POOL *pmem_pool, CPU_SIZE_T align,
			     LIB_ERR *perr)
{
	MEM_POOL *pmem_seg;
	...
	pmem_seg =
		pmem_pool->SegHeadPtr; /* Get mem pool's head seg.                             */
	pmem_seg_size =
		(pmem_seg->SegAddr != (void *)0) ?
			pmem_seg :
			&Mem_PoolHeap; /* See Note #1.                                         */
	size_rem =
		pmem_seg_size
			->SegSizeRem; /* Get mem seg's rem'ing mem size.                      */
	...
	if (align >
		...
		size_rem_mod =
			(seg_addr_mod > 0u) ? (align - seg_addr_mod) : 0u;
		size_rem -=
			size_rem_mod; /* ... adj rem'ing size by offset to align'd seg addr.  */
	}
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
void Mem_PoolCreate(MEM_POOL *pmem_pool, void *pmem_base_addr,
		    CPU_SIZE_T mem_size, MEM_POOL_BLK_QTY blk_nbr,
		    CPU_SIZE_T blk_size, CPU_SIZE_T blk_align,
		    CPU_SIZE_T *poctets_reqd, LIB_ERR *perr)
{
	MEM_POOL *pmem_pool_heap;
	...
	*poctets_reqd =
		0u; /* Init octets req'd for err (see Note #4).         */

	Mem_PoolClr(
		pmem_pool,
		perr); /* Init mem pool     for err (see Note #4).         */
	...
	if (pmem_base_addr ==
	    (void *)0) { /* If no base addr, cfg mem pool from heap.         */
		...
		pmem_addr_pool =
			pmem_addr_ptrs +
			size_tot_ptrs; /* Adj next avail addr for mem pool blks.           */
...
#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
		if (pmem_base_addr_end <
		    pmem_base_addr_start) { /* Chk ovf of end addr.                             */
			CPU_CRITICAL_EXIT();
		...
		while (pmem_seg_next !=
		       (MEM_POOL *)0) { /* Srch tbl for mem seg with same base addr/size.   */

			if ((pmem_base_addr ==
			     pmem_seg_next
				     ->SegAddr) && /* If same base addr/size found, ...                */
			    (mem_size == pmem_seg_next->SegSizeTot)) {
				pmem_seg =
					pmem_seg_next; /* ... mem seg already avail in tbl.                */
				break;

			} else {
				pmem_seg_addr_start =
					(CPU_INT08U *)pmem_seg_next->SegAddr;
				pmem_seg_addr_end =
					(CPU_INT08U *)pmem_seg_next->SegAddr +
					pmem_seg_next->SegSizeTot - 1;

				if (pmem_base_addr_end <
				    pmem_seg_addr_start) { /* If mem seg addr/size prior to next mem seg, ...  */
					break; /* ... new mem seg NOT avail in tbl.                */

					/* If mem seg overlaps prev mem seg(s) in tbl, ...  */
				} else if (((pmem_base_addr_start <=
					     pmem_seg_addr_start) &&
					    (pmem_base_addr_end >=
					     pmem_seg_addr_start)) ||
					   ((pmem_base_addr_start >=
					     pmem_seg_addr_start) &&
					    (pmem_base_addr_end <=
					     pmem_seg_addr_end)) ||
					   ((pmem_base_addr_start <=
		...
		if (pmem_seg ==
		    (MEM_POOL *)0) { /* If mem seg NOT found, add    new  mem seg.       */
			pmem_seg = pmem_pool;
		...
		if (size_tot_pool >
			...
			*poctets_reqd =
				size_tot_pool -
				size_rem; /* ... rtn add'l seg  size needed.                  */
	...
	ppool_ptr = (void **)Mem_SegAlloc((MEM_POOL *)pmem_pool_heap,
					  ...
					  (CPU_SIZE_T)sizeof(void *));
	if (ppool_ptr ==
	    (void **)0) { /* If mem pool ptrs alloc failed, ...               */
		size_rem = pmem_pool_heap->SegSizeRem;
		CPU_CRITICAL_EXIT();
	...
	for (i = 0u; i < (CPU_SIZE_T)blk_nbr;
	     i++) { /* Alloc mem blks from mem seg.                     */
		pmem_blk = (void *)Mem_SegAlloc(pmem_seg, blk_size, blk_align);
		if (pmem_blk ==
		    (void *)0) { /* If    mem blks alloc failed, ...                 */
			pmem_addr_pool =
				(CPU_INT08U *)pmem_seg->SegAddrNextAvail;
	...
	/* ------------- UPDATE MEM POOL TBL -------------- */
	if (pmem_seg ==
	    pmem_pool) { /* Add mem pool as new  mem pool tbl seg.           */
		/* Update cur  mem seg  links.                      */
		pmem_pool->SegPrevPtr = pmem_seg_prev;
		pmem_pool->SegNextPtr = pmem_seg_next;

		if (pmem_seg_prev !=
		    (MEM_POOL *)0) { /* Update prev mem seg  link.                       */
			pmem_seg_prev->SegNextPtr = pmem_pool;
		} else {
			Mem_PoolTbl =
				pmem_pool; /* Update      mem tbl.                             */
		}

		if (pmem_seg_next !=
		    (MEM_POOL *)0) { /* Update next mem seg  link.                       */
		...
		pmem_seg->PoolNextPtr =
			pmem_pool; /* Update prev mem pool link.                       */

		if (pmem_pool_next !=
		    (MEM_POOL *)0) { /* Update next mem pool link.                       */
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
MEM_POOL_BLK_QTY Mem_PoolBlkGetNbrAvail(MEM_POOL *pmem_pool, LIB_ERR *perr)
{
...
void *Mem_PoolBlkGet(MEM_POOL *pmem_pool, CPU_SIZE_T size, LIB_ERR *perr)
{
...
#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
	if (pmem_pool->Type !=
	    LIB_MEM_TYPE_POOL) { /* Validate mem pool type.                          */
		CPU_CRITICAL_EXIT();
		*perr = LIB_MEM_ERR_INVALID_POOL;
		return ((void *)0);
	}
	...
	if (size >
	    pmem_pool->BlkSize) { /* Validate req'd size <= mem pool blk size.        */
		CPU_CRITICAL_EXIT();
		*perr = LIB_MEM_ERR_INVALID_BLK_SIZE;
		return ((void *)0);
	}
	...
	if (pmem_pool->BlkIx <
	    1) { /* Validate mem pool as NOT empty.                  */
		CPU_CRITICAL_EXIT();
	...
	if (pmem_pool->BlkIx >
	    pmem_pool->BlkNbr) { /* Validate mem pool ix NOT corrupt.                */
		CPU_CRITICAL_EXIT();
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
void *Mem_PoolBlkGetUsedAtIx(MEM_POOL *pmem_pool, MEM_POOL_IX used_ix,
			     LIB_ERR *perr)
{
	MEM_POOL_IX blk_ix;
	void *pmem_blk;
	...
	if (pmem_pool->BlkIx >=
	    pmem_pool->BlkNbr) { /* Validate mem pool as NOT full.                   */
		CPU_CRITICAL_EXIT();
		*perr = LIB_MEM_ERR_INVALID_BLK_IX;
		return ((void *)0);
	}
	...
	if (blk_ix >=
	    pmem_pool->BlkNbr) { /* Validate ix range.                               */
		CPU_CRITICAL_EXIT();
		*perr = LIB_MEM_ERR_INVALID_BLK_IX;
		return ((void *)0);
	}
...
void Mem_PoolBlkFree(MEM_POOL *pmem_pool, void *pmem_blk, LIB_ERR *perr)
{
	...
	addr_valid = Mem_PoolBlkIsValidAddr(
		pmem_pool,
		pmem_blk); /* Validate mem blk as valid pool blk addr.         */
	...
	for (i = 0u; i < pmem_pool->BlkIx;
	     i++) { /* Validate mem blk  NOT already in pool.           */
	...
	if (pmem_pool->BlkIx >=
	    pmem_pool->BlkNbr) { /* Validate mem pool NOT already full.              */
		CPU_CRITICAL_EXIT();
	...
	addr_valid = DEF_NO;
	for (i = pmem_pool->BlkIx; i < pmem_pool->BlkNbr;
	     i++) { /* Find ix of mem blk to free.                      */
...
/*$PAGE*/
#if (LIB_MEM_CFG_ALLOC_EN == DEF_ENABLED)
MEM_POOL_IX Mem_PoolBlkIxGet(MEM_POOL *pmem_pool, void *pmem_blk, LIB_ERR *perr)
{
	void *p_addr;
	CPU_BOOLEAN addr_valid;
	MEM_POOL_IX i;
	...
	if (pmem_pool->BlkIx >=
	    pmem_pool->BlkNbr) { /* Validate mem pool NOT full.                      */
		CPU_CRITICAL_EXIT();
		*perr = LIB_MEM_ERR_POOL_FULL;
		return (invalid_ix);
	}
...
static CPU_BOOLEAN Mem_PoolBlkIsValidAddr(MEM_POOL *pmem_pool, void *pmem_blk)
{
	...
	if ((pmem_blk < ppool_addr_start) || (pmem_blk > ppool_addr_end)) {
		return (DEF_NO);
	}
	...
	ppool_addr_first = (CPU_INT08U *)((CPU_INT08U *)ppool_addr_start +
					  mem_align_offset);
				     ...
				     CPU_SIZE_T blk_size, CPU_SIZE_T blk_align)
{
	...
	size_tot = mem_align_offset +
		   ((blk_size + blk_align_offset) * ((CPU_SIZE_T)blk_nbr - 1)) +
		   blk_size;

#if (LIB_MEM_CFG_ARG_CHK_EXT_EN ==                                             \
     DEF_ENABLED) /* Chk ovf of tot size = A + [(B + C) * D] + E      */
	blk_size_mem_aligned =
		mem_align_offset +
		blk_size; /* Chk ovf of A + E :                               */
	...
	if (blk_nbr > 1) {
		...
		if ((blk_size_aligned <
		     blk_align_offset) || /* Chk ovf of      (B + C) :                        */
		    (blk_size_aligned < blk_size)) {
			return (0u);
		}

		blk_size_aligned_nbr =
			blk_size_aligned * ((CPU_SIZE_T)blk_nbr - 1);
		if ((blk_size_aligned_nbr <
		     blk_size_aligned) || /* Chk ovf of     [(B + C) * D] :                   */
		...
		if ((blk_size_tot <
		     blk_size_aligned_nbr) || /* Chk ovf of     [(B + C) * D] + E :               */
		    (blk_size_tot < blk_size)) {
			return (0u);
		}

		if ((size_tot <
		     blk_size_mem_aligned) || /* Chk ovf of A + [(B + C) * D] + E :               */
			  ...
			  CPU_SIZE_T align)
{
	...
	mem_align = (CPU_SIZE_T)(
		(CPU_ADDR)pmem_addr %
		align); /* Calc mem align.                                  */
	...
	if (size_tot >
	    pmem_pool->SegSizeRem) { /* If insufficient mem seg size rem, ...            */
		return ((
			void *)0); /* ... rtn NULL.                                    */
	}
	...
	pmem_addr +=
		align_offset; /* Align mem addr.                                  */

	pmem_pool->SegAddrNextAvail = (void *)
		pmem_addr_next; /* Adv next avail addr.                             */
	pmem_pool->SegSizeRem -= (CPU_SIZE_T)
		size_tot; /* Adj rem mem seg size.                            */
