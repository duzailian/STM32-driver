...
typedef CPU_TS32
	CPU_TS; /* Req'd for backwards-compatibility.                   */

#if (CPU_CFG_TS_TMR_EN ==                                                      \
     DEF_ENABLED) /* CPU ts tmr defined to cfg'd word size (see Note #1). */
...
#if (CPU_CFG_NAME_EN == DEF_ENABLED)
CPU_CORE_EXT CPU_CHAR CPU_Name
	[CPU_CFG_NAME_SIZE]; /* CPU host name.                                       */
#endif

#if ((CPU_CFG_TS_32_EN == DEF_ENABLED) &&                                      \
     (CPU_CFG_TS_TMR_SIZE < CPU_WORD_SIZE_32))
CPU_CORE_EXT CPU_TS32
	CPU_TS_32_Accum; /* 32-bit accum'd ts  (in ts tmr cnts).                 */
CPU_CORE_EXT CPU_TS_TMR
	CPU_TS_32_TmrPrev; /* 32-bit ts prev tmr (in ts tmr cnts).                 */
#endif

#if ((CPU_CFG_TS_64_EN == DEF_ENABLED) &&                                      \
     (CPU_CFG_TS_TMR_SIZE < CPU_WORD_SIZE_64))
CPU_CORE_EXT CPU_TS64
	CPU_TS_64_Accum; /* 64-bit accum'd ts  (in ts tmr cnts).                 */
CPU_CORE_EXT CPU_TS_TMR
	CPU_TS_64_TmrPrev; /* 64-bit ts prev tmr (in ts tmr cnts).                 */
#endif

#if (CPU_CFG_TS_TMR_EN == DEF_ENABLED)
CPU_CORE_EXT CPU_TS_TMR_FREQ
	CPU_TS_TmrFreq_Hz; /* CPU ts tmr freq (in Hz).                             */
#endif

#ifdef CPU_CFG_INT_DIS_MEAS_EN
CPU_CORE_EXT CPU_INT16U
	CPU_IntDisMeasCtr; /* Nbr tot    ints dis'd ctr.                           */
CPU_CORE_EXT CPU_INT16U
	CPU_IntDisNestCtr; /* Nbr nested ints dis'd ctr.                           */
/* Ints dis'd time (in ts tmr cnts) : ...               */
CPU_CORE_EXT CPU_TS_TMR
	CPU_IntDisMeasStart_cnts; /* ...  start time.                                     */
CPU_CORE_EXT CPU_TS_TMR
	CPU_IntDisMeasStop_cnts; /* ...  stop  time.                                     */
CPU_CORE_EXT CPU_TS_TMR
	CPU_IntDisMeasOvrhd_cnts; /* ...        time meas ovrhd.                          */
CPU_CORE_EXT CPU_TS_TMR
	CPU_IntDisMeasMaxCur_cnts; /* ...     resetable max time dis'd.                    */
CPU_CORE_EXT CPU_TS_TMR
	CPU_IntDisMeasMax_cnts; /* ... non-resetable max time dis'd.                    */
#endif
...
#define CPU_TYPE_CREATE(char_1, char_2, char_3, char_4)                        \
	(((CPU_INT32U)((CPU_INT08U)(char_1)) << (0u * DEF_OCTET_NBR_BITS)) |   \
...
#define CPU_TYPE_CREATE(char_1, char_2, char_3, char_4)                        \
	(((CPU_INT32U)((CPU_INT08U)(char_1)) << (2u * DEF_OCTET_NBR_BITS)) |   \
...
#elif ((CPU_CFG_NAME_EN != DEF_ENABLED) && (CPU_CFG_NAME_EN != DEF_DISABLED))
...
#elif (DEF_CHK_VAL(CPU_CFG_NAME_SIZE, 1, DEF_INT_08U_MAX_VAL) != DEF_OK)
...
#elif ((CPU_CFG_TS_32_EN != DEF_DISABLED) && (CPU_CFG_TS_32_EN != DEF_ENABLED))
...
#elif ((CPU_CFG_TS_64_EN != DEF_DISABLED) && (CPU_CFG_TS_64_EN != DEF_ENABLED))
...
#elif ((CPU_CFG_TS_EN != DEF_DISABLED) && (CPU_CFG_TS_EN != DEF_ENABLED))
...
#elif (DEF_CHK_VAL(CPU_CFG_INT_DIS_MEAS_OVRHD_NBR, CPU_TIME_MEAS_NBR_MIN,      \
