...
void CPU_BitBandClr(CPU_ADDR addr, CPU_INT08U bit_nbr)
{
	...
	if ((addr >= CPU_BIT_BAND_SRAM_REG_LO) &&
		...
		bit_word_off = ((addr - CPU_BIT_BAND_SRAM_REG_LO) * 32) +
			       (bit_nbr * 4);
		...
		*(volatile CPU_INT32U *)(bit_word_addr) = 0;
		...
		bit_word_off = ((addr - CPU_BIT_BAND_PERIPH_REG_LO) * 32) +
			       (bit_nbr * 4);
...
void CPU_IntSrcPrioSet(CPU_INT08U pos, CPU_INT08U prio)
{
	...
	case CPU_INT_SVCALL: /* SVCall.                                              */
		...
		temp &= ~((CPU_INT32U)DEF_OCTET_MASK
			  << (3 * DEF_OCTET_NBR_BITS));
...
CPU_INT16S CPU_IntSrcPrioGet(CPU_INT08U pos)
{
	...
	default:
		...
		if (pos <
			...
			prio = (temp >> (nbr * DEF_OCTET_NBR_BITS)) &
			       DEF_OCTET_MASK;
		} else {
			prio = DEF_INT_16S_MIN_VAL;
		}
