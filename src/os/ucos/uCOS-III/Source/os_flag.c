		  ...
		  OS_ERR *p_err)
{
	...
	OS_CRITICAL_ENTER();
	p_grp->Type =
		OS_OBJ_TYPE_FLAG; /* Set to event flag group type                           */
	p_grp->NamePtr = p_name;
	p_grp->Flags =
		flags; /* Set to desired initial value                           */
	p_grp->TS = (CPU_TS)0;
...
OS_OBJ_QTY OSFlagDel(OS_FLAG_GRP *p_grp, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if called from ISR ...                             */
		*p_err =
			OS_ERR_DEL_ISR; /* ... can't DELETE from an ISR                           */
		return ((OS_OBJ_QTY)0);
	}
#endif
	...
	if (p_grp->Type !=
	    OS_OBJ_TYPE_FLAG) { /* Validate event group object                            */
	...
	case OS_OPT_DEL_ALWAYS: /* Always delete the event flag group                     */
		...
		while (cnt >
		       0u) { /* Remove all tasks from the pend list                    */
			p_pend_data = p_pend_list->HeadPtr;
			p_tcb = p_pend_data->TCBPtr;
			OS_PendObjDel((OS_PEND_OBJ *)((void *)p_grp), p_tcb,
				      ts);
			cnt--;
		}
#if OS_CFG_DBG_EN > 0u
		    ...
		    OS_OPT opt, CPU_TS *p_ts, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if called from ISR ...                             */
		*p_err =
			OS_ERR_PEND_ISR; /* ... can't PEND from an ISR                             */
		return ((OS_FLAGS)0);
	}
#endif
...
#if OS_CFG_OBJ_TYPE_CHK_EN > 0u
	if (p_grp->Type !=
	    OS_OBJ_TYPE_FLAG) { /* Validate that we are pointing at an event flag         */
		*p_err = OS_ERR_OBJ_TYPE;
		return ((OS_FLAGS)0);
	}
#endif

	if ((opt & OS_OPT_PEND_FLAG_CONSUME) !=
	    (OS_OPT)0) { /* See if we need to consume the flags                    */
		consume = DEF_TRUE;
	} else {
		consume = DEF_FALSE;
	}
	...
	case OS_OPT_PEND_FLAG_SET_ALL: /* See if all required flags are set                      */
		flags_rdy = (OS_FLAGS)(
			p_grp->Flags &
			flags); /* Extract only the bits we want                          */
		if (flags_rdy ==
		    flags) { /* Must match ALL the bits that we want                   */
			if (consume ==
			    DEF_TRUE) { /* See if we need to consume the flags                    */
				p_grp->Flags &=
					~flags_rdy; /* Clear ONLY the flags that we wanted                    */
			}
			OSTCBCurPtr->FlagsRdy =
				flags_rdy; /* Save flags that were ready                             */
			if (p_ts != (CPU_TS *)0) {
				*p_ts = p_grp->TS;
			}
			...
			*p_err = OS_ERR_NONE;
			...
			if ((opt & OS_OPT_PEND_NON_BLOCKING) != (OS_OPT)0) {
				CPU_CRITICAL_EXIT();
				*p_err =
					OS_ERR_PEND_WOULD_BLOCK; /* Specified non-blocking so task would block             */
				return ((OS_FLAGS)0);
			} else { /* Specified blocking so check is scheduler is locked     */
				if (OSSchedLockNestingCtr >
				    (OS_NESTING_CTR)0) { /* See if called with scheduler locked ...      */
					CPU_CRITICAL_EXIT();
					*p_err =
						OS_ERR_SCHED_LOCKED; /* ... can't PEND when locked                   */
					return ((OS_FLAGS)0);
				}
			}
			/* Lock the scheduler/re-enable interrupts                */
			OS_CRITICAL_ENTER_CPU_EXIT();
			OS_FlagBlock(&pend_data, p_grp, flags, opt, timeout);
			OS_CRITICAL_EXIT_NO_SCHED();
		}
		break;
	...
	case OS_OPT_PEND_FLAG_SET_ANY:
		flags_rdy = (OS_FLAGS)(
			...
			flags); /* Extract only the bits we want                          */
		if (flags_rdy !=
		    (OS_FLAGS)0) { /* See if any flag set                                    */
			if (consume ==
			    DEF_TRUE) { /* See if we need to consume the flags                    */
				p_grp->Flags &=
					~flags_rdy; /* Clear ONLY the flags that we got                       */
			}
			OSTCBCurPtr->FlagsRdy =
				flags_rdy; /* Save flags that were ready                             */
			if (p_ts != (CPU_TS *)0) {
	...
	case OS_OPT_PEND_FLAG_CLR_ALL: /* See if all required flags are cleared                  */
		...
		if (flags_rdy ==
			...
			if (consume ==
			    DEF_TRUE) { /* See if we need to consume the flags                    */
				p_grp->Flags |=
					flags_rdy; /* Set ONLY the flags that we wanted                      */
			}
			OSTCBCurPtr->FlagsRdy =
				flags_rdy; /* Save flags that were ready                             */
			if (p_ts != (CPU_TS *)0) {
	...
	case OS_OPT_PEND_FLAG_CLR_ANY:
		flags_rdy = (OS_FLAGS)(
			...
			flags); /* Extract only the bits we want                          */
		if (flags_rdy !=
		    (OS_FLAGS)0) { /* See if any flag cleared                                */
			if (consume ==
			    DEF_TRUE) { /* See if we need to consume the flags                    */
				p_grp->Flags |=
					flags_rdy; /* Set ONLY the flags that we got                         */
			}
			OSTCBCurPtr->FlagsRdy =
				flags_rdy; /* Save flags that were ready                             */
			if (p_ts != (CPU_TS *)0) {
	...
	CPU_CRITICAL_EXIT();
	*p_err =
		OS_ERR_NONE; /* Event(s) must have occurred                            */
	return (flags_rdy);
}
...
OS_OBJ_QTY OSFlagPendAbort(OS_FLAG_GRP *p_grp, OS_OPT opt, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0u) { /* Not allowed to Pend Abort from an ISR                  */
...
#if OS_CFG_OBJ_TYPE_CHK_EN > 0u
	if (p_grp->Type !=
	    OS_OBJ_TYPE_FLAG) { /* Make sure event flag group was created                 */
		*p_err = OS_ERR_OBJ_TYPE;
		return ((OS_OBJ_QTY)0u);
	}
#endif
	...
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0u) { /* Any task waiting on flag group?                        */
	...
	while (p_pend_list->NbrEntries > (OS_OBJ_QTY)0u) {
		...
		OS_PendAbort((OS_PEND_OBJ *)((void *)p_grp), p_tcb, ts);
		nbr_tasks++;
		if (opt !=
		    OS_OPT_PEND_ABORT_ALL) { /* Pend abort all tasks waiting?                          */
			break; /* No                                                     */
		}
	}
	OS_CRITICAL_EXIT_NO_SCHED();
...
OS_FLAGS OSFlagPendGetFlagsRdy(OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if called from ISR ...                             */
		*p_err =
			OS_ERR_PEND_ISR; /* ... can't get from an ISR                              */
		return ((OS_FLAGS)0);
	}
#endif
		    ...
		    OS_ERR *p_err)
{
...
#if OS_CFG_OBJ_TYPE_CHK_EN > 0u
	if (p_grp->Type !=
	    OS_OBJ_TYPE_FLAG) { /* Make sure we are pointing to an event flag grp         */
		*p_err = OS_ERR_OBJ_TYPE;
		return ((OS_FLAGS)0);
	}
#endif
	...
	if (OSIntNestingCtr >
		...
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_FLAG, /* Post to ISR queue                                      */
			(void *)p_grp, (void *)0, (OS_MSG_SIZE)0,
			(OS_FLAGS)flags, (OS_OPT)opt, (CPU_TS)ts,
			(OS_ERR *)p_err);
		return ((OS_FLAGS)0);
	}
#endif

	flags_cur = OS_FlagPost(p_grp, flags, opt, ts, p_err);
...
{
	OSTCBCurPtr->FlagsPend =
		flags; /* Save the flags that we need to wait for                */
	OSTCBCurPtr->FlagsOpt =
		opt; /* Save the type of wait we are doing                     */
	OSTCBCurPtr->FlagsRdy = (OS_FLAGS)0;

	OS_Pend(p_pend_data, (OS_PEND_OBJ *)((void *)p_grp),
		OS_TASK_PEND_ON_FLAG, timeout);
}
...
void OS_FlagClr(OS_FLAG_GRP *p_grp)
{
	...
	p_grp->NamePtr = (CPU_CHAR *)((
		void *)"?FLAG"); /* Unknown name                                       */
		     ...
		     OS_ERR *p_err)
{
	...
	case OS_OPT_POST_FLAG_SET | OS_OPT_POST_NO_SCHED:
		p_grp->Flags |=
			flags; /* Set   the flags specified in the group             */
		break;

	case OS_OPT_POST_FLAG_CLR:
	case OS_OPT_POST_FLAG_CLR | OS_OPT_POST_NO_SCHED:
		p_grp->Flags &=
			~flags; /* Clear the flags specified in the group             */
		break;
	...
	p_pend_list = &p_grp->PendList;
	if (p_pend_list->NbrEntries ==
	    0u) { /* Any task waiting on event flag group?              */
		CPU_CRITICAL_EXIT(); /* No                                                 */
		*p_err = OS_ERR_NONE;
		return (p_grp->Flags);
	}
	...
	p_tcb = p_pend_data->TCBPtr;
	while (p_tcb !=
	       (OS_TCB *)0) { /* Go through all tasks waiting on event flag(s)      */
		...
		case OS_OPT_PEND_FLAG_SET_ALL: /* See if all req. flags are set for current node     */
			...
			if (flags_rdy == p_tcb->FlagsPend) {
				OS_FlagTaskRdy(
					p_tcb, /* Make task RTR, event(s) Rx'd                       */
					flags_rdy, ts);
			}
			break;
		...
		case OS_OPT_PEND_FLAG_CLR_ALL: /* See if all req. flags are set for current node     */
			flags_rdy =
				(OS_FLAGS)(~p_grp->Flags & p_tcb->FlagsPend);
			if (flags_rdy == p_tcb->FlagsPend) {
				OS_FlagTaskRdy(
					p_tcb, /* Make task RTR, event(s) Rx'd                       */
					flags_rdy, ts);
		...
		p_pend_data =
			p_pend_data_next; /* Point to next task waiting for event flag(s)       */
