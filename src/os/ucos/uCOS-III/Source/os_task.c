...
void OSTaskChangePrio(OS_TCB *p_tcb, OS_PRIO prio_new, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* ---------- CANNOT CREATE A TASK FROM AN ISR ---------- */
	...
	if (prio_new ==
	    0) { /* Cannot set to IntQueue Task priority                   */
	...
	if (prio_new >=
	    (OS_CFG_PRIO_MAX -
	     1u)) { /* Cannot set to Idle Task priority                       */
		*p_err = OS_ERR_PRIO_INVALID;
		return;
	}

	if (p_tcb ==
	    (OS_TCB *)0) { /* See if want to change priority of 'self'               */
		CPU_CRITICAL_ENTER();
		p_tcb = OSTCBCurPtr;
		CPU_CRITICAL_EXIT();
		self = DEF_TRUE;
	...
	case OS_TASK_STATE_RDY:
		OS_RdyListRemove(
			p_tcb); /* Remove from current priority                           */
		p_tcb->Prio =
			prio_new; /* Set new task priority                                  */
	...
	case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
		...
		case OS_TASK_PEND_ON_SEM:
			OS_PendListChangePrio(p_tcb, prio_new);
			break;
		  ...
		  OS_OPT opt, OS_ERR *p_err)
{
	...
	0u /* ---------------- VALIDATE ARGUMENTS ------------------ */
	if (p_tcb ==
	    (OS_TCB *)0) { /* User must supply a valid OS_TCB                        */
		*p_err = OS_ERR_TCB_INVALID;
		return;
	}
	if (p_task ==
	    (OS_TASK_PTR)0) { /* User must supply a valid task                          */
		*p_err = OS_ERR_TASK_INVALID;
		return;
	}
	if (p_stk_base ==
	    (CPU_STK *)0) { /* User must supply a valid stack base address            */
		*p_err = OS_ERR_STK_INVALID;
		return;
	}
	if (stk_size <
	    OSCfg_StkSizeMin) { /* User must supply a valid minimum stack size            */
		*p_err = OS_ERR_STK_SIZE_INVALID;
		return;
	}
	if (stk_limit >=
	    stk_size) { /* User must supply a valid stack limit                   */
		*p_err = OS_ERR_STK_LIMIT_INVALID;
		return;
	}
	if (prio >=
	    OS_CFG_PRIO_MAX) { /* Priority must be within 0 and OS_CFG_PRIO_MAX-1        */
		*p_err = OS_ERR_PRIO_INVALID;
		return;
	}
#endif
	...
	if (prio == (OS_PRIO)0) {
		if (p_tcb != &OSIntQTaskTCB) {
			*p_err =
				OS_ERR_PRIO_INVALID; /* Not allowed to use priority 0                          */
			return;
		}
	}
#endif
	...
	if (prio == (OS_CFG_PRIO_MAX - 1u)) {
		if (p_tcb != &OSIdleTaskTCB) {
			*p_err =
				OS_ERR_PRIO_INVALID; /* Not allowed to use same priority as idle task          */
			return;
		}
	}

	OS_TaskInitTCB(
		p_tcb); /* Initialize the TCB to default values                   */

	*p_err = OS_ERR_NONE;
	/* --------------- CLEAR THE TASK'S STACK --------------- */
	if ((opt & OS_OPT_TASK_STK_CHK) !=
	    (OS_OPT)0) { /* See if stack checking has been enabled                 */
		if ((opt & OS_OPT_TASK_STK_CLR) !=
		    (OS_OPT)0) { /* See if stack needs to be cleared                       */
			p_sp = p_stk_base;
			for (i = 0u; i < stk_size;
			     i++) { /* Stack grows from HIGH to LOW memory                    */
	...
	p_sp = OSTaskStkInit(p_task, p_arg, p_stk_base, p_stk_limit, stk_size,
			     opt);

	/* -------------- INITIALIZE THE TCB FIELDS ------------- */
	p_tcb->TaskEntryAddr =
		p_task; /* Save task entry point address                          */
	p_tcb->TaskEntryArg =
		p_arg; /* Save task entry argument                               */

	p_tcb->NamePtr =
		p_name; /* Save task name                                         */

	p_tcb->Prio =
		prio; /* Save the task's priority                               */

	p_tcb->StkPtr =
		p_sp; /* Save the new top-of-stack pointer                      */
	p_tcb->StkLimitPtr =
		p_stk_limit; /* Save the stack limit pointer                           */

	p_tcb->TimeQuanta =
		time_quanta; /* Save the #ticks for time slice (0 means not sliced)    */
	...
	OS_MsgQInit(
		&p_tcb->MsgQ, /* Initialize the task's message queue                    */
		q_size);
#else
	(void)&q_size;
#endif
	...
	if (OSRunning !=
	    OS_STATE_OS_RUNNING) { /* Return if multitasking has not started                 */
		OS_CRITICAL_EXIT();
		return;
	}
...
void OSTaskDel(OS_TCB *p_tcb, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if trying to delete from ISR                       */
	...
	if (p_tcb ==
	    &OSIdleTaskTCB) { /* Not allowed to delete the idle task                    */
...
#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
	if (p_tcb ==
	    &OSIntQTaskTCB) { /* Cannot delete the ISR handler task                     */
...
#if OS_CFG_TASK_Q_EN > 0u
	(void)OS_MsgQFreeAll(
		&p_tcb->MsgQ); /* Free task's message queue messages                     */
#endif
	...
	OS_TaskInitTCB(
		p_tcb); /* Initialize the TCB to default values                   */
	p_tcb->TaskState = (OS_STATE)
		OS_TASK_STATE_DEL; /* Indicate that the task was deleted                     */

	OS_CRITICAL_EXIT_NO_SCHED();

	*p_err =
		OS_ERR_NONE; /* See Note #1.                                           */
...
OS_MSG_QTY OSTaskQFlush(OS_TCB *p_tcb, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Can't flush a message queue from an ISR                */
	...
	if (p_tcb ==
	    (OS_TCB *)0) { /* Flush message queue of calling task?                   */
		CPU_CRITICAL_ENTER();
		p_tcb = OSTCBCurPtr;
		CPU_CRITICAL_EXIT();
	}
	...
	OS_CRITICAL_ENTER();
	entries = OS_MsgQFreeAll(
		&p_tcb->MsgQ); /* Return all OS_MSGs to the OS_MSG pool                  */
	OS_CRITICAL_EXIT();
	*p_err = OS_ERR_NONE;
	return (entries);
}
		  ...
		  CPU_TS *p_ts, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Can't Pend from an ISR                                 */
	...
	if (p_msg_size ==
	    (OS_MSG_SIZE
		     *)0) { /* User must supply a valid destination for msg size      */
	...
	p_msg_q =
		&OSTCBCurPtr
			 ->MsgQ; /* Any message waiting in the message queue?              */
	p_void = OS_MsgQGet(p_msg_q, p_msg_size, p_ts, p_err);
	if (*p_err == OS_ERR_NONE) {
#if OS_CFG_TASK_PROFILE_EN > 0u
		if (p_ts != (CPU_TS *)0) {
			OSTCBCurPtr->MsgQPendTime = OS_TS_GET() - *p_ts;
			if (OSTCBCurPtr->MsgQPendTimeMax <
			    OSTCBCurPtr->MsgQPendTime) {
				OSTCBCurPtr->MsgQPendTimeMax =
					OSTCBCurPtr->MsgQPendTime;
			}
		}
#endif
	...
	if ((opt & OS_OPT_PEND_NON_BLOCKING) !=
	    (OS_OPT)0) { /* Caller wants to block if not available?                */
		*p_err =
			OS_ERR_PEND_WOULD_BLOCK; /* No                                                     */
		CPU_CRITICAL_EXIT();
		return ((void *)0);
	} else { /* Yes                                                    */
		if (OSSchedLockNestingCtr >
		    (OS_NESTING_CTR)0) { /*     Can't block when the scheduler is locked           */
			CPU_CRITICAL_EXIT();
	...
	OS_Pend((OS_PEND_DATA
			 *)0, /* Block task pending on Message                          */
		(OS_PEND_OBJ *)0, (OS_STATE)OS_TASK_PEND_ON_TASK_Q,
		(OS_TICK)timeout);
	OS_CRITICAL_EXIT_NO_SCHED();
...
CPU_BOOLEAN OSTaskQPendAbort(OS_TCB *p_tcb, OS_OPT opt, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if called from ISR ...                             */
		*p_err =
			OS_ERR_PEND_ABORT_ISR; /* ... can't Pend Abort from an ISR                       */
		return (DEF_FALSE);
	}
#endif
	...
	if ((p_tcb ==
	     (OS_TCB *)0) || /* Pend abort self?                                       */
	    (p_tcb == OSTCBCurPtr)) {
		CPU_CRITICAL_EXIT();
		*p_err =
			OS_ERR_PEND_ABORT_SELF; /* ... doesn't make sense                                 */
		return (DEF_FALSE);
	}
#endif

	if (p_tcb->PendOn !=
	    OS_TASK_PEND_ON_TASK_Q) { /* Is task waiting for a message?                         */
		 ...
		 OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr > (OS_NESTING_CTR)0) {
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_TASK_MSG, /* Post to ISR queue                                      */
			(void *)p_tcb, (void *)p_void, (OS_MSG_SIZE)msg_size,
			(OS_FLAGS)0, (OS_OPT)opt, (CPU_TS)ts, (OS_ERR *)p_err);
		return;
	}
#endif

	OS_TaskQPost(p_tcb, p_void, msg_size, opt, ts, p_err);
}
#endif
...
OS_REG_ID OSTaskRegGetID(OS_ERR *p_err)
{
	...
	if (OSTaskRegNextAvailID >=
	    OS_CFG_TASK_REG_TBL_SIZE) { /* See if we exceeded the number of IDs available   */
		*p_err =
			OS_ERR_NO_MORE_ID_AVAIL; /* Yes, cannot allocate more task register IDs      */
		CPU_CRITICAL_EXIT();
		return ((OS_REG_ID)OS_CFG_TASK_REG_TBL_SIZE);
	}
...
#if OS_CFG_ARG_CHK_EN > 0u
	if ((p_tcb ==
	     (OS_TCB *)0) || /* We cannot resume 'self'                                */
	    (p_tcb == OSTCBCurPtr)) {
		CPU_CRITICAL_EXIT();
...
#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if called from an ISR                              */
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_TASK_RESUME, /* Post to ISR queue                                      */
			(void *)p_tcb, (void *)0, (OS_MSG_SIZE)0, (OS_FLAGS)0,
			(OS_OPT)0, (CPU_TS)0, (OS_ERR *)p_err);
		return;
	}
#endif
			 ...
			 OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	if (OSTCBCurPtr->SemCtr >
	    (OS_SEM_CTR)0) { /* See if task already been signaled                      */
		OSTCBCurPtr->SemCtr--;
	...
	if ((opt & OS_OPT_PEND_NON_BLOCKING) !=
		...
		*p_err =
		...
		if (OSSchedLockNestingCtr >
		    (OS_NESTING_CTR)0) { /* Can't pend when the scheduler is locked                */
			CPU_CRITICAL_EXIT();
			*p_err = OS_ERR_SCHED_LOCKED;
			return ((OS_SEM_CTR)0);
		}
	...
	OS_CRITICAL_ENTER_CPU_EXIT();
	OS_Pend((OS_PEND_DATA
			 *)0, /* Block task pending on Signal                           */
		(OS_PEND_OBJ *)0, (OS_STATE)OS_TASK_PEND_ON_TASK_SEM,
		(OS_TICK)timeout);
	OS_CRITICAL_EXIT_NO_SCHED();
	...
	CPU_CRITICAL_ENTER();
	switch (OSTCBCurPtr
			->PendStatus) { /* See if we timed-out or aborted                         */
	case OS_STATUS_PEND_OK:
		if (p_ts != (CPU_TS *)0) {
			*p_ts = OSTCBCurPtr->TS;
#if OS_CFG_TASK_PROFILE_EN > 0u
			OSTCBCurPtr->SemPendTime =
				OS_TS_GET() - OSTCBCurPtr->TS;
			if (OSTCBCurPtr->SemPendTimeMax <
			    OSTCBCurPtr->SemPendTime) {
				OSTCBCurPtr->SemPendTimeMax =
					OSTCBCurPtr->SemPendTime;
			}
#endif
		}
	...
	case OS_STATUS_PEND_ABORT:
		...
		*p_err =
			OS_ERR_PEND_ABORT; /* Indicate that we aborted                               */
		break;
	...
	case OS_STATUS_PEND_TIMEOUT:
		...
		*p_err =
			OS_ERR_TIMEOUT; /* Indicate that we didn't get event within TO            */
		break;
...
CPU_BOOLEAN OSTaskSemPendAbort(OS_TCB *p_tcb, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (p_tcb->PendOn !=
	    OS_TASK_PEND_ON_TASK_SEM) { /* Is task waiting for a signal?                          */
		CPU_CRITICAL_EXIT();
		*p_err = OS_ERR_PEND_ABORT_NONE;
		return (DEF_FALSE);
	}
	...
	OS_CRITICAL_ENTER();
	ts = OS_TS_GET();
	OS_PendAbort((OS_PEND_OBJ *)0, p_tcb, ts);
	OS_CRITICAL_EXIT_NO_SCHED();
	if ((opt & OS_OPT_POST_NO_SCHED) == (OS_OPT)0) {
		OSSched(); /* Run the scheduler                                      */
	}
...
OS_SEM_CTR OSTaskSemPost(OS_TCB *p_tcb, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
		...
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_TASK_SIGNAL, /* Post to ISR queue                                      */
			(void *)p_tcb, (void *)0, (OS_MSG_SIZE)0, (OS_FLAGS)0,
			(OS_OPT)0, (CPU_TS)ts, (OS_ERR *)p_err);
		return ((OS_SEM_CTR)0);
	}
#endif

	ctr = OS_TaskSemPost(p_tcb, opt, ts, p_err);
		  ...
		  OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if trying to check stack from ISR                  */
	...
	if (p_free ==
	    (CPU_STK_SIZE
		     *)0) { /* User must specify valid destinations for the sizes     */
		*p_err = OS_ERR_PTR_INVALID;
		return;
	}
	...
	CPU_CRITICAL_ENTER();
	if (p_tcb ==
	    (OS_TCB *)0) { /* Check the stack of the current task?                   */
		p_tcb = OSTCBCurPtr; /* Yes                                                    */
	}

	if (p_tcb->StkPtr ==
	    (CPU_STK *)0) { /* Make sure task exist                                   */
		CPU_CRITICAL_EXIT();
	...
	if ((p_tcb->Opt & OS_OPT_TASK_STK_CHK) ==
	    (OS_OPT)0) { /* Make sure stack checking option is set                 */
		CPU_CRITICAL_EXIT();
		*p_free = (CPU_STK_SIZE)0;
		*p_used = (CPU_STK_SIZE)0;
	...
	while (*p_stk ==
	       (CPU_STK)0) { /* Compute the number of zero entries on the stk          */
		p_stk++;
		free_stk++;
	}
#else
	p_stk = p_tcb->StkBasePtr + p_tcb->StkSize -
		1u; /* Start at the highest memory and go down                */
	...
	*p_used =
		(p_tcb->StkSize -
		 free_stk); /* Compute number of entries used on the stack            */
	*p_err = OS_ERR_NONE;
}
#endif
...
void OSTaskSuspend(OS_TCB *p_tcb, OS_ERR *p_err)
{
	...
	if (p_tcb ==
	    &OSIdleTaskTCB) { /* Make sure not suspending the idle task                 */
...
#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
	if (p_tcb ==
	    &OSIntQTaskTCB) { /* Not allowed to suspend the ISR handler task            */
	...
	if (OSIntNestingCtr >
		...
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_TASK_SUSPEND, /* Post to ISR queue                                      */
			(void *)p_tcb, (void *)0, (OS_MSG_SIZE)0, (OS_FLAGS)0,
			(OS_OPT)0, (CPU_TS)0, (OS_ERR *)p_err);
		return;
	}
...
void OS_TaskInit(OS_ERR *p_err)
{
	...
	OSTaskQty =
		(OS_OBJ_QTY)0; /* Clear the number of tasks                              */
	OSTaskCtxSwCtr =
		(OS_CTX_SW_CTR)0; /* Clear the context switch counter                       */
...
#if OS_CFG_TASK_Q_EN > 0u
	OS_MsgQInit(&p_tcb->MsgQ, (OS_MSG_QTY)0u);
#if OS_CFG_TASK_PROFILE_EN > 0u
		  ...
		  CPU_TS ts, OS_ERR *p_err)
{
	...
	OS_CRITICAL_ENTER();
	if (p_tcb ==
	    (OS_TCB *)0) { /* Post msg to 'self'?                                    */
		p_tcb = OSTCBCurPtr;
	}
	*p_err =
		OS_ERR_NONE; /* Assume we won't have any errors                        */
	switch (p_tcb->TaskState) {
	case OS_TASK_STATE_RDY:
	...
	case OS_TASK_STATE_DLY_SUSPENDED:
		OS_MsgQPut(
			&p_tcb->MsgQ, /* Deposit the message in the queue                       */
			p_void, msg_size, opt, ts, p_err);
		OS_CRITICAL_EXIT();
		break;
	...
	case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
		if (p_tcb->PendOn ==
		    OS_TASK_PEND_ON_TASK_Q) { /* Is task waiting for a message to be sent to it?        */
			OS_Post((OS_PEND_OBJ *)0, p_tcb, p_void, msg_size, ts);
			OS_CRITICAL_EXIT_NO_SCHED();
			...
			OS_MsgQPut(
				&p_tcb->MsgQ, /* No,  Task is pending on something else ...             */
				p_void, /*      ... Deposit the message in the task's queue       */
				msg_size, opt, ts, p_err);
			OS_CRITICAL_EXIT();
		}
		break;
...
void OS_TaskReturn(void)
{
	...
	OSTaskReturnHook(
		OSTCBCurPtr); /* Call hook to let user decide on what to do             */
#if OS_CFG_TASK_DEL_EN > 0u
	OSTaskDel(
		(OS_TCB *)0, /* Delete task if it accidentally returns!                */
		(OS_ERR *)&err);
#else
	for (;;) {
		OSTimeDly((OS_TICK)OSCfg_TickRate_Hz, (OS_OPT)OS_OPT_TIME_DLY,
			  (OS_ERR *)&err);
	}
#endif
}
...
OS_SEM_CTR OS_TaskSemPost(OS_TCB *p_tcb, OS_OPT opt, CPU_TS ts, OS_ERR *p_err)
{
	...
	*p_err =
	...
	case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
		if (p_tcb->PendOn ==
		    OS_TASK_PEND_ON_TASK_SEM) { /* Is task signaled waiting for a signal?             */
			OS_Post((OS_PEND_OBJ
					 *)0, /*      Task is pending on signal                     */
				(OS_TCB *)p_tcb, (void *)0, (OS_MSG_SIZE)0u,
				(CPU_TS)ts);
			ctr = p_tcb->SemCtr;
			OS_CRITICAL_EXIT_NO_SCHED();
			if ((opt & OS_OPT_POST_NO_SCHED) == (OS_OPT)0) {
...
void OS_TaskSuspend(OS_TCB *p_tcb, OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	if (p_tcb ==
	    (OS_TCB *)0) { /* See if specified to suspend self                       */
		p_tcb = OSTCBCurPtr;
	}

	if (p_tcb == OSTCBCurPtr) {
		if (OSSchedLockNestingCtr >
		    (OS_NESTING_CTR)0) { /* Can't suspend when the scheduler is locked             */
			CPU_CRITICAL_EXIT();
			*p_err = OS_ERR_SCHED_LOCKED;
			return;
		}
