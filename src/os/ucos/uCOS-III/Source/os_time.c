...
void OSTimeDly(OS_TICK dly, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0u) { /* Not allowed to call from an ISR                        */
	...
	if (OSSchedLockNestingCtr >
	    (OS_NESTING_CTR)0u) { /* Can't delay when the scheduler is locked               */
	...
	OS_TickListInsert(OSTCBCurPtr, dly, opt, p_err);
	if (*p_err != OS_ERR_NONE) {
		OS_CRITICAL_EXIT_NO_SCHED();
		return;
	}
	OS_RdyListRemove(
		OSTCBCurPtr); /* Remove current task from ready list                    */
	OS_CRITICAL_EXIT_NO_SCHED();
		   ...
		   CPU_INT32U milli, OS_OPT opt, OS_ERR *p_err)
{
	...
	opt_time =
		opt &
		OS_OPT_TIME_MASK; /* Retrieve time options only.                            */
	switch (opt_time) {
	case OS_OPT_TIME_DLY:
	case OS_OPT_TIME_TIMEOUT:
	case OS_OPT_TIME_PERIODIC:
		if (milli ==
		    (CPU_INT32U)0u) { /* Make sure we didn't specify a 0 delay                  */
	...
	0u /* Validate arguments to be within range                  */
	...
	ticks = ((OS_TICK)hours * (OS_TICK)3600u +
		 (OS_TICK)minutes * (OS_TICK)60u + (OS_TICK)seconds) *
			tick_rate +
		(tick_rate * ((OS_TICK)milli + (OS_TICK)500u / tick_rate)) /
			(OS_TICK)1000u;

	if (ticks > (OS_TICK)0u) {
		OS_CRITICAL_ENTER();
		OSTCBCurPtr->TaskState = OS_TASK_STATE_DLY;
		OS_TickListInsert(OSTCBCurPtr, ticks, opt_time, p_err);
		if (*p_err != OS_ERR_NONE) {
			OS_CRITICAL_EXIT_NO_SCHED();
			return;
		}
...
void OSTimeDlyResume(OS_TCB *p_tcb, OS_ERR *p_err)
{
...
#if OS_CFG_ARG_CHK_EN > 0u
	if (p_tcb ==
	    (OS_TCB *)0) { /* Not possible for the running task to be delayed!       */
	...
	CPU_CRITICAL_ENTER();
	if (p_tcb ==
	    OSTCBCurPtr) { /* Not possible for the running task to be delayed!       */
		*p_err = OS_ERR_TASK_NOT_DLY;
		CPU_CRITICAL_EXIT();
		return;
	}
	...
	case OS_TASK_STATE_DLY:
		...
		OS_TickListRemove(
			p_tcb); /* Remove task from tick list                             */
		OS_RdyListInsert(
			p_tcb); /* Add to ready list                                      */
		OS_CRITICAL_EXIT_NO_SCHED();
		*p_err = OS_ERR_NONE;
		break;
...
void OSTimeTick(void)
{
	...
	OS_IntQPost(
		(OS_OBJ_TYPE)
			OS_OBJ_TYPE_TICK, /* Post to ISR queue                                      */
		(void *)&OSRdyList[OSPrioCur], (void *)0, (OS_MSG_SIZE)0u,
		(OS_FLAGS)0u, (OS_OPT)0u, (CPU_TS)ts, (OS_ERR *)&err);
