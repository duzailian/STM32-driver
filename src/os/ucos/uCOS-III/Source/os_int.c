		 ...
		 OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	if (OSIntQNbrEntries <
	    OSCfg_IntQSize) { /* Make sure we haven't already filled the ISR queue      */
		OSIntQNbrEntries++;
		...
		OSIntQInPtr->Type =
			type; /* Save object type being posted                          */
		OSIntQInPtr->ObjPtr =
			p_obj; /* Save pointer to object being posted                    */
		OSIntQInPtr->MsgPtr =
			p_void; /* Save pointer to message if posting to a message queue  */
		OSIntQInPtr->MsgSize =
			msg_size; /* Save the message size   if posting to a message queue  */
		OSIntQInPtr->Flags =
			flags; /* Save the flags if posting to an event flag group       */
		OSIntQInPtr->Opt =
			opt; /* Save post options                                      */
		OSIntQInPtr->TS =
			ts; /* Save time stamp                                        */

		OSIntQInPtr =
			OSIntQInPtr
				->NextPtr; /* Point to the next interrupt handler queue entry        */

		OSRdyList[0].NbrEntries =
			(OS_OBJ_QTY)1; /* Make the interrupt handler task ready to run           */
		OSRdyList[0].HeadPtr = &OSIntQTaskTCB;
		OSRdyList[0].TailPtr = &OSIntQTaskTCB;
		OS_PrioInsert(
			0u); /* Add task priority 0 in the priority table              */
		if (OSPrioCur !=
		    0) { /* Chk if OSIntQTask is not running                       */
			OSPrioSaved =
				OSPrioCur; /* Save current priority                                  */
		}
...
void OS_IntQRePost(void)
{
	...
	case OS_OBJ_TYPE_FLAG:
		...
		(void)OS_FlagPost((OS_FLAG_GRP *)OSIntQOutPtr->ObjPtr,
				  ...
				  (CPU_TS)OSIntQOutPtr->TS, (OS_ERR *)&err);
#endif
		break;
	...
	case OS_OBJ_TYPE_Q:
		...
		OS_QPost((OS_Q *)OSIntQOutPtr->ObjPtr,
			 ...
			 (OS_OPT)OSIntQOutPtr->Opt, (CPU_TS)OSIntQOutPtr->TS,
			 (OS_ERR *)&err);
#endif
		break;
	...
	case OS_OBJ_TYPE_TICK:
		...
		(void)OS_TaskSemPost(
			(OS_TCB *)&OSTickTaskTCB, /* Signal tick task                        */
			(OS_OPT)OS_OPT_POST_NONE, (CPU_TS)OSIntQOutPtr->TS,
			(OS_ERR *)&err);
#if OS_CFG_TMR_EN > 0u
		OSTmrUpdateCtr--;
		if (OSTmrUpdateCtr == (OS_CTR)0u) {
			...
			(void)OS_TaskSemPost(
				(OS_TCB *)&OSTmrTaskTCB, /* Signal timer task                       */
				(OS_OPT)OS_OPT_POST_NONE, (CPU_TS)ts,
				(OS_ERR *)&err);
		}
#endif
		break;
	...
	while (DEF_ON) {
		...
		while (done == DEF_FALSE) {
			...
			if (OSIntQNbrEntries == (OS_OBJ_QTY)0u) {
				OSRdyList[0].NbrEntries =
					(OS_OBJ_QTY)0u; /* Remove from ready list                                 */
				OSRdyList[0].HeadPtr = (OS_TCB *)0;
				OSRdyList[0].TailPtr = (OS_TCB *)0;
				OS_PrioRemove(
					0u); /* Remove from the priority table                         */
				CPU_CRITICAL_EXIT();
				OSSched();
				...
				OSIntQOutPtr =
					OSIntQOutPtr
						->NextPtr; /* Point to next item in the ISR queue                    */
				OSIntQNbrEntries--;
				CPU_CRITICAL_EXIT();
			}
		}
...
void OS_IntQTaskInit(OS_ERR *p_err)
{
	...
	OSIntQOvfCtr =
		(OS_QTY)0u; /* Clear the ISR queue overflow counter                   */
	...
	p_int_q =
		OSCfg_IntQBasePtr; /* Initialize the circular ISR queue                      */
	...
	OSTaskCreate(
		...
		(OS_TASK_PTR)OS_IntQTask, (void *)0,
		...
		(CPU_STK_SIZE)OSCfg_IntQTaskStkSize, (OS_MSG_QTY)0u,
		(OS_TICK)0u, (void *)0,
