...
void OSStatReset(OS_ERR *p_err)
{
	...
	OSIntQTaskTimeMax =
		(CPU_TS)0; /* Reset the task execution times                         */
	OSIntQNbrEntriesMax =
		(OS_OBJ_QTY)0; /* Reset the queue maximum number of entries              */
#endif
	...
	OSIntDisTimeMax =
		(CPU_TS)0; /* Reset the maximum interrupt disable time               */
#endif

#if OS_CFG_SCHED_LOCK_TIME_MEAS_EN > 0u
	OSSchedLockTimeMax =
		(CPU_TS)0; /* Reset the maximum scheduler lock time                  */
#endif
	...
	CPU_CRITICAL_EXIT();
	while (p_tcb !=
	       (OS_TCB *)0) { /* Reset per-Task statistics                              */
		CPU_CRITICAL_ENTER();
	...
	CPU_CRITICAL_EXIT();
	while (p_q !=
	       (OS_Q *)0) { /* Reset message queues statistics                        */
		CPU_CRITICAL_ENTER();
		p_msg_q = &p_q->MsgQ;
		p_msg_q->NbrEntriesMax = (OS_MSG_QTY)0;
		p_q = p_q->DbgNextPtr;
...
void OSStatTaskCPUUsageInit(OS_ERR *p_err)
{
	...
	OSTimeDly(
		(OS_TICK)2, /* Synchronize with clock tick                            */
		(OS_OPT)OS_OPT_TIME_DLY, (OS_ERR *)&err);
	if (err != OS_ERR_NONE) {
		*p_err = err;
		return;
	}
	...
	OSTimeDly(
		dly, /* Determine MAX. idle counter value                      */
		OS_OPT_TIME_DLY, &err);
	...
	OSStatTaskCtrMax =
		OSStatTaskCtr; /* Store maximum idle counter count                       */
...
void OS_StatTask(void *p_arg)
{
	...
	while (OSStatTaskRdy != DEF_TRUE) {
		OSTimeDly(
			2u * OSCfg_StatTaskRate_Hz, /* Wait until statistic task is ready                     */
			OS_OPT_TIME_DLY, &err);
	}
	OSStatReset(
		&err); /* Reset statistics                                       */
	...
	while (DEF_ON) {
		...
		OSStatTaskCtrRun =
			OSStatTaskCtr; /* Obtain the of the stat counter for the past .1 second  */
		OSStatTaskCtr =
			(OS_TICK)0; /* Reset the stat counter for the next .1 second          */
		CPU_CRITICAL_EXIT();

		if (OSStatTaskCtrMax >
		    OSStatTaskCtrRun) { /* Compute CPU Usage with best resolution                 */
			if (OSStatTaskCtrMax <
			    400000u) { /*            1 to       400,000 */
				ctr_mult = 10000u;
				ctr_div = 1u;
			} else if (OSStatTaskCtrMax <
				   4000000u) { /*      400,000 to     4,000,000 */
				ctr_mult = 1000u;
				ctr_div = 10u;
			} else if (OSStatTaskCtrMax <
				   40000000u) { /*    4,000,000 to    40,000,000 */
				ctr_mult = 100u;
				ctr_div = 100u;
			} else if (OSStatTaskCtrMax <
				   400000000u) { /*   40,000,000 to   400,000,000 */
				ctr_mult = 10u;
				ctr_div = 1000u;
			...
			OSStatTaskCPUUsage = (OS_CPU_USAGE)(
				(OS_TICK)10000u -
		...
		CPU_CRITICAL_EXIT();
		while (p_tcb !=
		       (OS_TCB *)0) { /* ----------------- TOTAL CYCLES COUNT ----------------- */
			OS_CRITICAL_ENTER();
			p_tcb->CyclesTotalPrev =
				p_tcb->CyclesTotal; /* Save accumulated # cycles into a temp variable         */
			p_tcb->CyclesTotal =
				(OS_CYCLES)0; /* Reset total cycles for task for next run               */
			OS_CRITICAL_EXIT();

			cycles_total +=
				p_tcb->CyclesTotalPrev; /* Perform sum of all task # cycles                       */
		...
		if (cycles_total >
		    (OS_CYCLES)0u) { /* 'cycles_total' scaling ...    */
			if (cycles_total <
			    400000u) { /*            1 to       400,000 */
				cycles_mult = 10000u;
				cycles_div = 1u;
			} else if (cycles_total <
				   4000000u) { /*      400,000 to     4,000,000 */
				cycles_mult = 1000u;
				cycles_div = 10u;
			} else if (cycles_total <
...
#if OS_CFG_TASK_PROFILE_EN >                                                   \
	0u /* Compute execution time of each task                    */
			usage = (OS_CPU_USAGE)(cycles_mult *
					       p_tcb->CyclesTotalPrev /
					       cycles_max);
			if (usage > 10000u) {
				usage = 10000u;
			...
			if (p_tcb->CPUUsageMax <
			    usage) { /* Detect peak CPU usage                                  */
			...
			OSTaskStkChk(
				p_tcb, /* Compute stack usage of active tasks only               */
				&p_tcb->StkFree, &p_tcb->StkUsed, &err);
#endif
		...
		if (OSStatResetFlag ==
		    DEF_TRUE) { /* Check if need to reset statistics                      */
		...
		ts_end =
			OS_TS_GET() -
			ts_start; /* Measure execution time of statistic task               */
...
void OS_StatTaskInit(OS_ERR *p_err)
{
	...
	OSStatTaskRdy =
		OS_STATE_NOT_RDY; /* Statistic task is not ready                            */
	OSStatResetFlag = DEF_FALSE;
