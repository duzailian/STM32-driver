...
CPU_DATA OSPrioTbl
	[OS_PRIO_TBL_SIZE]; /* Declare the array local to this file to allow for  ... */
...
OS_PRIO OS_PrioGetHighest(void)
{
	...
	p_tbl = &OSPrioTbl[0];
	while (*p_tbl ==
	       (CPU_DATA)0) { /* Search the bitmap table for the highest priority       */
		prio += DEF_INT_CPU_NBR_BITS; /* Compute the step of each CPU_DATA entry                */
		p_tbl++;
	}
	prio += (OS_PRIO)CPU_CntLeadZeros(
		*p_tbl); /* Find the position of the first bit set at the entry    */
	return (prio);
}
