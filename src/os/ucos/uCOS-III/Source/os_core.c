...
void OSInit(OS_ERR *p_err)
{
	...
	OSIntNestingCtr =
		(OS_NESTING_CTR)0; /* Clear the interrupt nesting counter                    */

	OSRunning =
		OS_STATE_OS_STOPPED; /* Indicate that multitasking not started                 */

	OSSchedLockNestingCtr =
		(OS_NESTING_CTR)0; /* Clear the scheduling lock counter                      */

	OSTCBCurPtr =
		(OS_TCB *)0; /* Initialize OS_TCB pointers to a known state            */
	OSTCBHighRdyPtr = (OS_TCB *)0;

	OSPrioCur =
		(OS_PRIO)0; /* Initialize priority variables to a known state         */
	...
	OS_AppTaskCreateHookPtr =
		(OS_APP_HOOK_TCB)0; /* Clear application hook pointers                        */
...
#if OS_CFG_FLAG_EN >                                                           \
	0u /* Initialize the Event Flag module                       */
	OS_FlagInit(p_err);
...
#if OS_CFG_MEM_EN >                                                            \
	0u /* Initialize the Memory Manager module                   */
	OS_MemInit(p_err);
	if (*p_err != OS_ERR_NONE) {
		return;
	}
...
#if (OS_MSG_EN) >                                                              \
	0u /* Initialize the free list of OS_MSGs                    */
...
#if OS_CFG_MUTEX_EN >                                                          \
	0u /* Initialize the Mutex Manager module                    */
	OS_MutexInit(p_err);
	if (*p_err != OS_ERR_NONE) {
		return;
	}
...
#if OS_CFG_Q_EN > 0u
	OS_QInit(
		p_err); /* Initialize the Message Queue Manager module            */
	if (*p_err != OS_ERR_NONE) {
		return;
	}
#endif
...
#if OS_CFG_SEM_EN >                                                            \
	0u /* Initialize the Semaphore Manager module                */
	OS_SemInit(p_err);
	if (*p_err != OS_ERR_NONE) {
		return;
	}
	...
	OS_TLS_Init(
		p_err); /* Initialize Task Local Storage, before creating tasks   */
	if (*p_err != OS_ERR_NONE) {
		return;
	}
#endif
	...
	OS_TaskInit(
		p_err); /* Initialize the task manager                            */
	if (*p_err != OS_ERR_NONE) {
		return;
	}
	...
	OS_IntQTaskInit(
		p_err); /* Initialize the Interrupt Queue Handler Task            */
	if (*p_err != OS_ERR_NONE) {
		return;
	}
#endif
	...
	OS_IdleTaskInit(
		p_err); /* Initialize the Idle Task                               */
	if (*p_err != OS_ERR_NONE) {
		return;
	}

	OS_TickTaskInit(
		p_err); /* Initialize the Tick Task                               */
	if (*p_err != OS_ERR_NONE) {
		return;
	}

#if OS_CFG_STAT_TASK_EN >                                                      \
	0u /* Initialize the Statistic Task                          */
...
#if OS_CFG_TMR_EN >                                                            \
	0u /* Initialize the Timer Manager module                    */
	OS_TmrInit(p_err);
	if (*p_err != OS_ERR_NONE) {
		return;
	}
...
{
	if (OSRunning !=
	    OS_STATE_OS_RUNNING) { /* Is OS running?                                         */
		return; /* No                                                     */
	}

	if (OSIntNestingCtr >=
	    (OS_NESTING_CTR)250u) { /* Have we nested past 250 levels?                        */
		return; /* Yes                                                    */
	}
...
void OSIntExit(void)
{
	...
	if (OSRunning !=
	    OS_STATE_OS_RUNNING) { /* Has the OS started?                                    */
		return; /* No                                                     */
	}

	CPU_INT_DIS();
	if (OSIntNestingCtr ==
	    (OS_NESTING_CTR)0) { /* Prevent OSIntNestingCtr from wrapping                  */
		CPU_INT_EN();
		return;
	}
	OSIntNestingCtr--;
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* ISRs still nested?                                     */
		CPU_INT_EN(); /* Yes                                                    */
		return;
	}

	if (OSSchedLockNestingCtr >
	    (OS_NESTING_CTR)0) { /* Scheduler still locked?                                */
		CPU_INT_EN(); /* Yes                                                    */
		return;
	}
	...
	OSTCBHighRdyPtr =
		OSRdyList[OSPrioHighRdy]
			.HeadPtr; /* Get highest priority task ready-to-run                 */
	if (OSTCBHighRdyPtr ==
	    OSTCBCurPtr) { /* Current task still the highest priority?               */
		CPU_INT_EN(); /* Yes                                                    */
		return;
	}
	...
	OSTCBHighRdyPtr
		->CtxSwCtr++; /* Inc. # of context switches for this new task           */
#endif
...
void OSSched(void)
{
	...
	if (OSSchedLockNestingCtr >
	    (OS_NESTING_CTR)0) { /* Scheduler locked?                                      */
		return; /* Yes                                                    */
	}
	...
	if (OSTCBHighRdyPtr ==
	    OSTCBCurPtr) { /* Current task is still highest priority task?           */
...
#if OS_CFG_TASK_PROFILE_EN > 0u
	OSTCBHighRdyPtr
		->CtxSwCtr++; /* Inc. # of context switches to this task                */
#endif
...
void OSSchedLock(OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Not allowed to call from an ISR                        */
	...
	if (OSRunning !=
	    OS_STATE_OS_RUNNING) { /* Make sure multitasking is running                      */
		*p_err = OS_ERR_OS_NOT_RUNNING;
		return;
	}

	if (OSSchedLockNestingCtr >=
	    (OS_NESTING_CTR)250u) { /* Prevent OSSchedLockNestingCtr overflowing              */
...
void OSSchedUnlock(OS_ERR *p_err)
{
	...
	if (OSSchedLockNestingCtr ==
	    (OS_NESTING_CTR)0) { /* See if the scheduler is locked                         */
			  ...
			  OS_ERR *p_err)
{
	...
	if (dflt_time_quanta > (OS_TICK)0) {
		...
		OSSchedRoundRobinDfltTimeQuanta =
			(OS_TICK)(OSCfg_TickRate_Hz / (OS_RATE_HZ)10);
	}
	CPU_CRITICAL_EXIT();
	*p_err = OS_ERR_NONE;
}
...
void OSSchedRoundRobinYield(OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Can't call this function from an ISR                   */
	...
	if (OSSchedLockNestingCtr >
	    (OS_NESTING_CTR)0) { /* Can't yield if the scheduler is locked                 */
		*p_err = OS_ERR_SCHED_LOCKED;
		return;
	}

	if (OSSchedRoundRobinEn !=
	    DEF_TRUE) { /* Make sure round-robin has been enabled                 */
	...
	CPU_CRITICAL_ENTER();
	p_rdy_list =
		&OSRdyList[OSPrioCur]; /* Can't yield if it's the only task at that priority     */
	...
	OS_RdyListMoveHeadToTail(
		p_rdy_list); /* Move current OS_TCB to the end of the list             */
	p_tcb = p_rdy_list
			->HeadPtr; /* Point to new OS_TCB at head of the list                */
	if (p_tcb->TimeQuanta ==
	    (OS_TICK)0) { /* See if we need to use the default time slice           */
		p_tcb->TimeQuantaCtr = OSSchedRoundRobinDfltTimeQuanta;
	} else {
		p_tcb->TimeQuantaCtr =
			p_tcb->TimeQuanta; /* Load time slice counter with new time                  */
	}
...
void OSStart(OS_ERR *p_err)
{
	...
	if (OSRunning == OS_STATE_OS_STOPPED) {
		...
		*p_err =
			OS_ERR_FATAL_RETURN; /* OSStart() is not supposed to return                    */
	} else {
		*p_err =
			OS_ERR_OS_RUNNING; /* OS is already running                                  */
	}
}
...
void OS_IdleTaskInit(OS_ERR *p_err)
{
	...
	OSTaskCreate((OS_TCB *)&OSIdleTaskTCB,
		     ...
		     (OS_TASK_PTR)OS_IdleTask, (void *)0,
		     (OS_PRIO)(OS_CFG_PRIO_MAX - 1u),
		     (CPU_STK *)OSCfg_IdleTaskStkBasePtr,
		     (CPU_STK_SIZE)OSCfg_IdleTaskStkLimit,
		     (CPU_STK_SIZE)OSCfg_IdleTaskStkSize, (OS_MSG_QTY)0u,
		     (OS_TICK)0u, (void *)0,
	     ...
	     OS_TICK timeout)
{
	...
	OSTCBCurPtr->PendOn =
		pending_on; /* Resource not available, wait until it is              */
	OSTCBCurPtr->PendStatus = OS_STATUS_PEND_OK;

	OS_TaskBlock(
		OSTCBCurPtr, /* Block the task and add it to the tick list if needed  */
		timeout);

	if (p_obj !=
	    (OS_PEND_OBJ
		     *)0) { /* Add the current task to the pend list ...             */
		p_pend_list =
			&p_obj->PendList; /* ... if there is an object to pend on                  */
		p_pend_data->PendObjPtr =
			p_obj; /* Save the pointer to the object pending on             */
		OS_PendDataInit(
			(OS_TCB *)
				OSTCBCurPtr, /* Initialize the remaining field                        */
			(OS_PEND_DATA *)p_pend_data, (OS_OBJ_QTY)1);
		OS_PendListInsertPrio(
			p_pend_list, /* Insert in the pend list in priority order             */
			p_pend_data);
	} else {
		OSTCBCurPtr->PendDataTblEntries =
			(OS_OBJ_QTY)0; /* If no object being pended on the clear these fields   */
		OSTCBCurPtr->PendDataTblPtr =
			(OS_PEND_DATA
				 *)0; /* ... in the TCB                                        */
	}
#if OS_CFG_DBG_EN > 0u
	OS_PendDbgNameAdd(p_obj, OSTCBCurPtr);
#endif
}
...
void OS_PendAbort(OS_PEND_OBJ *p_obj, OS_TCB *p_tcb, CPU_TS ts)
{
	...
	case OS_TASK_STATE_PEND_TIMEOUT:
		if (p_tcb->PendOn == OS_TASK_PEND_ON_MULTI) {
			OS_PendAbort1(
				p_obj, /* Indicate which object was pend aborted            */
				p_tcb, ts);
		}
#if (OS_MSG_EN > 0u)
		p_tcb->MsgPtr = (void *)0;
		...
		if (p_obj != (OS_PEND_OBJ *)0) {
			OS_PendListRemove(
				p_tcb); /* Remove task from all pend lists                   */
		}
		OS_TaskRdy(p_tcb);
		p_tcb->TaskState =
			OS_TASK_STATE_RDY; /* Task will be ready                                */
		p_tcb->PendStatus =
			OS_STATUS_PEND_ABORT; /* Indicate pend was aborted                         */
		p_tcb->PendOn =
			OS_TASK_PEND_ON_NOTHING; /* Indicate no longer pending                        */
		break;
	...
	case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
		...
		OS_TickListRemove(
			p_tcb); /* Cancel the timeout                                */
		p_tcb->TaskState =
			OS_TASK_STATE_SUSPENDED; /* Pend Aborted task is still suspended              */
		p_tcb->PendStatus =
			OS_STATUS_PEND_ABORT; /* Indicate pend was aborted                         */
		p_tcb->PendOn =
			OS_TASK_PEND_ON_NOTHING; /* Indicate no longer pending                        */
...
{
	OS_OBJ_QTY
		n_pend_list; /* Number of pend lists                           */
	OS_PEND_DATA *p_pend_data;

	p_pend_data =
		p_tcb->PendDataTblPtr; /* Point to the first OS_PEND_DATA to remove      */
	n_pend_list =
		p_tcb->PendDataTblEntries; /* Get number of entries in the table             */

	while (n_pend_list >
	       (OS_OBJ_QTY)0) { /* Mark posted object in OS_PEND_DATA table       */
		if (p_obj ==
		    p_pend_data
			    ->PendObjPtr) { /* Did we find the object pend aborted?           */
			p_pend_data->RdyObjPtr =
				p_obj; /* Yes, indicate the object in the .RdyObjPtr     */
			p_pend_data->RdyTS =
				ts; /*      save the timestamp of the pend abort      */
			break;
		}
		p_pend_data++;
		n_pend_list--;
		     ...
		     OS_OBJ_QTY tbl_size)
{
	...
	p_tcb->PendDataTblEntries =
		tbl_size; /* Link the TCB to the beginning of the table             */
	p_tcb->PendDataTblPtr = p_pend_data_tbl;

	for (i = 0u; i < tbl_size; i++) {
		p_pend_data_tbl->NextPtr =
			(OS_PEND_DATA
				 *)0; /* Initialize all the fields                              */
		...
		p_pend_data_tbl->TCBPtr =
			p_tcb; /* Every entry points back to the TCB of the task         */
		p_pend_data_tbl++;
	}
}
...
void OS_PendDbgNameAdd(OS_PEND_OBJ *p_obj, OS_TCB *p_tcb)
{
	...
	if (p_obj != (OS_PEND_OBJ *)0) {
		p_tcb->DbgNamePtr =
			p_obj->NamePtr; /* Task pending on this object ... save name in TCB       */
		p_pend_list =
			&p_obj->PendList; /* Find name of HP task pending on this object ...        */
		p_pend_data = p_pend_list->HeadPtr;
		p_tcb1 = p_pend_data->TCBPtr;
		p_obj->DbgNamePtr =
			p_tcb1->NamePtr; /* ... Save in object                                     */
	} else {
		switch (p_tcb->PendOn) {
...
void OS_PendDbgNameRemove(OS_PEND_OBJ *p_obj, OS_TCB *p_tcb)
{
	...
	p_tcb->DbgNamePtr = (CPU_CHAR *)((
		void *)" "); /* Remove name of object pended on for readied task       */
	...
	if (p_pend_data != (OS_PEND_DATA *)0) {
		...
		p_obj->DbgNamePtr = (CPU_CHAR *)((
			void *)" "); /* No other task pending on object                        */
	}
}
#endif
...
void OS_PendListChangePrio(OS_TCB *p_tcb, OS_PRIO prio_new)
{
	...
	p_tcb->Prio = prio_new;
	p_pend_data =
		p_tcb->PendDataTblPtr; /* Point to first wait list entry                 */
	n_pend_list =
		p_tcb->PendDataTblEntries; /* Get the number of pend list task is in         */

	while (n_pend_list > 0u) {
		p_obj = p_pend_data
				->PendObjPtr; /* Get pointer to pend list                       */
		p_pend_list = &p_obj->PendList;
		if (p_pend_list->NbrEntries >
		    1u) { /* Only move if multiple entries in the list      */
			OS_PendListRemove1(
				p_pend_list, /* Remove entry from current position             */
				p_pend_data);
			OS_PendListInsertPrio(
				p_pend_list, /* INSERT it back in the list                     */
				p_pend_data);
		}
...
void OS_PendListInsertHead(OS_PEND_LIST *p_pend_list, OS_PEND_DATA *p_pend_data)
{
	...
	p_pend_list
		->NbrEntries++; /* One more entry in the list                             */
	p_pend_data->NextPtr =
		p_pend_list
			->HeadPtr; /* Adjust new entry's links                               */
	p_pend_data->PrevPtr = (OS_PEND_DATA *)0;
	p_pend_data_next =
		p_pend_list
			->HeadPtr; /* Adjust old head of list's links                        */
	if (p_pend_data_next !=
	    (OS_PEND_DATA
		     *)0) { /* See if we already have a head to replace               */
		p_pend_data_next->PrevPtr =
			p_pend_data; /* Yes, point to new entry                                */
	}
	p_pend_list->HeadPtr =
		p_pend_data; /* We have a new list head                                */
...
void OS_PendListInsertPrio(OS_PEND_LIST *p_pend_list, OS_PEND_DATA *p_pend_data)
{
	...
	p_tcb = p_pend_data
			->TCBPtr; /* Obtain the priority of the task to insert    */
	prio = p_tcb->Prio;
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0) { /* CASE 0: Insert when there are no entries     */
		p_pend_list->NbrEntries =
			(OS_OBJ_QTY)1; /*         This is the first entry              */
		p_pend_data->NextPtr =
			(OS_PEND_DATA
				 *)0; /*         No other OS_PEND_DATAs in the list   */
		p_pend_data->PrevPtr = (OS_PEND_DATA *)0;
		p_pend_list->HeadPtr =
			p_pend_data; /*                                              */
		p_pend_list->TailPtr = p_pend_data;
		...
		p_pend_list
			->NbrEntries++; /* CASE 1: One more OS_PEND_DATA in the list    */
		p_pend_data_next = p_pend_list->HeadPtr;
		while (p_pend_data_next !=
		       (OS_PEND_DATA
				*)0) { /*         Find the position where to insert    */
			...
			if (prio < p_tcb_next->Prio) {
				...
				p_pend_data_next =
					p_pend_data_next
						->NextPtr; /*         Not Found, follow the list           */
			}
		}
		if (p_pend_data_next ==
		    (OS_PEND_DATA
			     *)0) { /*         TCB to insert is lower in prio       */
			p_pend_data->NextPtr =
				(OS_PEND_DATA
					 *)0; /*         ... insert at the tail.              */
			p_pend_data_prev = p_pend_list->TailPtr;
			p_pend_data->PrevPtr = p_pend_data_prev;
			p_pend_data_prev->NextPtr = p_pend_data;
			p_pend_list->TailPtr = p_pend_data;
		} else {
			if (p_pend_data_next->PrevPtr ==
			    (OS_PEND_DATA
				     *)0) { /*         Is new TCB highest priority?         */
				p_pend_data_next->PrevPtr =
					p_pend_data; /*         Yes, insert as new Head of list      */
				p_pend_data->PrevPtr = (OS_PEND_DATA *)0;
				...
				p_pend_data_prev =
					p_pend_data_next
						->PrevPtr; /*        No,  insert in between two entries   */
				p_pend_data->PrevPtr = p_pend_data_prev;
				p_pend_data->NextPtr = p_pend_data_next;
				p_pend_data_prev->NextPtr = p_pend_data;
...
void OS_PendListRemove(OS_TCB *p_tcb)
{
	...
	while (n_pend_list > (OS_OBJ_QTY)0) {
		p_obj = p_pend_data
				->PendObjPtr; /* Get pointer to pend list                       */
		p_pend_list = &p_obj->PendList;
		OS_PendListRemove1(p_pend_list, p_pend_data);
		p_pend_data++;
		n_pend_list--;
	}
...
void OS_PendListRemove1(OS_PEND_LIST *p_pend_list, OS_PEND_DATA *p_pend_data)
{
	...
	if (p_pend_list->NbrEntries == 1u) {
		p_pend_list->HeadPtr =
			(OS_PEND_DATA
				 *)0; /* Only one entry in the pend list                        */
		p_pend_list->TailPtr = (OS_PEND_DATA *)0;

	} else if (p_pend_data->PrevPtr ==
		   (OS_PEND_DATA
			    *)0) { /* See if entry is at the head of the list                */
		p_next =
			p_pend_data
				->NextPtr; /* Yes                                                    */
	...
	} else if (p_pend_data->NextPtr ==
		   (OS_PEND_DATA
			    *)0) { /* See if entry is at the tail of the list                */
		p_prev =
			p_pend_data
				->PrevPtr; /* Yes                                                    */
		...
		p_prev =
			p_pend_data
				->PrevPtr; /* Remove from inside the list                            */
		p_next = p_pend_data->NextPtr;
		p_prev->NextPtr = p_next;
		p_next->PrevPtr = p_prev;
	}
	p_pend_list
		->NbrEntries--; /* One less entry in the list                             */
...
void OS_PendObjDel(OS_PEND_OBJ *p_obj, OS_TCB *p_tcb, CPU_TS ts)
{
	...
	case OS_TASK_STATE_PEND_TIMEOUT:
		...
		p_tcb->TS = ts;
		OS_PendListRemove(
			p_tcb); /* Remove task from all wait lists                   */
		OS_TaskRdy(p_tcb);
		p_tcb->TaskState =
			OS_TASK_STATE_RDY; /* Task is readied because object is deleted         */
	...
	case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
		...
		OS_PendListRemove(
			p_tcb); /* Remove task from all wait lists                   */
		p_tcb->TaskState =
			OS_TASK_STATE_SUSPENDED; /* Task needs to remain suspended                    */
		p_tcb->PendStatus = OS_STATUS_PEND_DEL;
		p_tcb->PendOn =
			OS_TASK_PEND_ON_NOTHING; /* Indicate no longer pending                        */
		break;
...
void OS_PendObjDel1(OS_PEND_OBJ *p_obj, OS_TCB *p_tcb, CPU_TS ts)
{
	...
	while (n_pend_list >
		...
		if (p_obj ==
		    p_pend_data
			    ->PendObjPtr) { /* Did we find the object deleted?                */
			p_pend_data->RdyObjPtr =
				p_obj; /* Yes, indicate the object in the .RdyObjPtr     */
			p_pend_data->RdyTS =
				ts; /*      save the timestamp                        */
			break;
		}
		p_pend_data++;
		n_pend_list--;
	     ...
	     OS_MSG_SIZE msg_size, CPU_TS ts)
{
	...
	case OS_TASK_STATE_PEND_TIMEOUT:
		if (p_tcb->PendOn == OS_TASK_PEND_ON_MULTI) {
			OS_Post1(
				p_obj, /* Indicate which object was posted to               */
				p_tcb, p_void, msg_size, ts);
		} else {
#if (OS_MSG_EN > 0u)
			p_tcb->MsgPtr =
				p_void; /* Deposit message in OS_TCB of task waiting         */
			p_tcb->MsgSize =
				msg_size; /* ... assuming posting a message                    */
#endif
			p_tcb->TS = ts;
		}
		if (p_obj != (OS_PEND_OBJ *)0) {
			OS_PendListRemove(
				p_tcb); /* Remove task from wait list(s)                     */
#if OS_CFG_DBG_EN > 0u
			OS_PendDbgNameRemove(p_obj, p_tcb);
#endif
		}
		OS_TaskRdy(
			p_tcb); /* Make task ready to run                            */
		p_tcb->TaskState = OS_TASK_STATE_RDY;
		p_tcb->PendStatus =
			OS_STATUS_PEND_OK; /* Clear pend status                                 */
		p_tcb->PendOn =
			OS_TASK_PEND_ON_NOTHING; /* Indicate no longer pending                        */
		break;
	      ...
	      OS_MSG_SIZE msg_size, CPU_TS ts)
{
	...
	while (n_pend_list >
		...
		if (p_obj ==
		    p_pend_data
			    ->PendObjPtr) { /* Did we find the object posted to?              */
			p_pend_data->RdyObjPtr =
				p_obj; /* Yes, indicate the object in the .RdyObjPtr     */
			p_pend_data->RdyMsgPtr =
				p_void; /*      store the message posted                  */
			p_pend_data->RdyMsgSize =
				msg_size; /*      store the size of the message posted      */
			p_pend_data->RdyTS =
				ts; /*      save the timestamp of the post            */
			break;
		}
		p_pend_data++;
		n_pend_list--;
...
void OS_RdyListInit(void)
{
	...
	for (i = 0u; i < OS_CFG_PRIO_MAX;
	     i++) { /* Initialize the array of OS_RDY_LIST at each priority   */
...
void OS_RdyListInsert(OS_TCB *p_tcb)
{
	...
	if (p_tcb->Prio ==
	    OSPrioCur) { /* Are we readying a task at the same prio?               */
		OS_RdyListInsertTail(
			p_tcb); /* Yes, insert readied task at the end of the list        */
	} else {
		OS_RdyListInsertHead(
			p_tcb); /* No,  insert readied task at the beginning of the list  */
	}
}
...
void OS_RdyListInsertHead(OS_TCB *p_tcb)
{
	...
	if (p_rdy_list->NbrEntries ==
		...
		p_rdy_list->NbrEntries =
			(OS_OBJ_QTY)1; /*         This is the first entry                        */
		p_tcb->NextPtr =
			(OS_TCB *)0; /*         No other OS_TCBs in the list                   */
		p_tcb->PrevPtr = (OS_TCB *)0;
		p_rdy_list->HeadPtr =
			p_tcb; /*         Both list pointers point to this OS_TCB        */
		p_rdy_list->TailPtr = p_tcb;
	} else { /* CASE 1: Insert BEFORE the current head of list         */
		p_rdy_list
			->NbrEntries++; /*         One more OS_TCB in the list                    */
		p_tcb->NextPtr =
			p_rdy_list
				->HeadPtr; /*         Adjust new OS_TCBs links                       */
		p_tcb->PrevPtr = (OS_TCB *)0;
		p_tcb2 =
			p_rdy_list
				->HeadPtr; /*         Adjust old head of list's links                */
...
void OS_RdyListInsertTail(OS_TCB *p_tcb)
{
	...
	if (p_rdy_list->NbrEntries ==
		...
		p_rdy_list
			->NbrEntries++; /*         One more OS_TCB in the list                    */
		p_tcb->NextPtr =
			(OS_TCB *)0; /*         Adjust new OS_TCBs links                       */
		p_tcb2 = p_rdy_list->TailPtr;
		p_tcb->PrevPtr = p_tcb2;
		p_tcb2->NextPtr =
			p_tcb; /*         Adjust old tail of list's links                */
		p_rdy_list->TailPtr = p_tcb;
	}
}
...
void OS_RdyListMoveHeadToTail(OS_RDY_LIST *p_rdy_list)
{
	...
	case 2: /* SWAP the TCBs                                          */
		p_tcb1 =
			p_rdy_list
				->HeadPtr; /* Point to current head                                  */
		p_tcb2 =
			p_rdy_list
				->TailPtr; /* Point to current tail                                  */
	...
	default: /* Move only if there are more than 2 OS_TCBs in the list */
		...
		p_tcb2 =
			p_rdy_list
				->TailPtr; /* Point to current tail                                  */
		p_tcb3 =
			p_tcb1->NextPtr; /* Point to new list head                                 */
		p_tcb3->PrevPtr =
			(OS_TCB *)0; /* Adjust back    link of new list head                   */
		p_tcb1->NextPtr =
			(OS_TCB *)0; /* Adjust forward link of new list tail                   */
		p_tcb1->PrevPtr =
			p_tcb2; /* Adjust back    link of new list tail                   */
		p_tcb2->NextPtr =
			p_tcb1; /* Adjust forward link of old list tail                   */
		p_rdy_list->HeadPtr =
			p_tcb3; /* Adjust new list head and tail pointers                 */
		p_rdy_list->TailPtr = p_tcb1;
		break;
	}
}
...
void OS_RdyListRemove(OS_TCB *p_tcb)
{
	...
	p_rdy_list = &OSRdyList[p_tcb->Prio];
	p_tcb1 =
		p_tcb->PrevPtr; /* Point to next and previous OS_TCB in the list          */
	p_tcb2 = p_tcb->NextPtr;
	if (p_tcb1 ==
	    (OS_TCB *)0) { /* Was the OS_TCB to remove was at the head?              */
		if (p_tcb2 ==
		    (OS_TCB *)0) { /* Yes, was it the only OS_TCB?                           */
			p_rdy_list->NbrEntries =
				(OS_OBJ_QTY)0; /*      Yes, no more entries                              */
			p_rdy_list->HeadPtr = (OS_TCB *)0;
			p_rdy_list->TailPtr = (OS_TCB *)0;
			OS_PrioRemove(p_tcb->Prio);
		} else {
			p_rdy_list
				->NbrEntries--; /*      No,  one less entry                               */
			p_tcb2->PrevPtr =
				(OS_TCB *)0; /*           adjust back link of new list head            */
			p_rdy_list->HeadPtr =
				p_tcb2; /*           adjust OS_RDY_LIST's new head                */
		}
	} else {
		p_rdy_list
			->NbrEntries--; /* No,  one less entry                                    */
		...
		if (p_tcb2 == (OS_TCB *)0) {
			p_rdy_list->TailPtr =
				p_tcb1; /*      Removing the TCB at the tail, adj the tail ptr    */
		} else {
...
void OS_Sched0(void)
{
	...
	CPU_INT_DIS();
	OSPrioHighRdy =
		(OS_PRIO)0; /* Force the priority to 0                                */
	OSTCBHighRdyPtr =
		&OSIntQTaskTCB; /* Always schedule the ISR handler task                   */
#if OS_CFG_TASK_PROFILE_EN > 0u
	OSTCBHighRdyPtr
		->CtxSwCtr++; /* Inc. # of context switches to this task                */
#endif
...
void OS_SchedLockTimeMeasStop(void)
{
	...
	if (OSSchedLockNestingCtr ==
	    (OS_NESTING_CTR)0) { /* Make sure we fully un-nested scheduler lock            */
		delta = CPU_TS_TmrRd() /* Compute the delta time between begin and end           */
			- OSSchedLockTimeBegin;
		if (OSSchedLockTimeMax <
		    delta) { /* Detect peak value                                      */
			OSSchedLockTimeMax = delta;
		}
		if (OSSchedLockTimeMaxCur <
		    delta) { /* Detect peak value (for resettable value)               */
...
void OS_SchedRoundRobin(OS_RDY_LIST *p_rdy_list)
{
	...
	CPU_CRITICAL_ENTER();
	p_tcb = p_rdy_list
			->HeadPtr; /* Decrement time quanta counter                          */
	...
	if (p_tcb->TimeQuantaCtr >
	    (OS_TICK)0) { /* Task not done with its time quanta                     */
		CPU_CRITICAL_EXIT();
		return;
	}

	if (p_rdy_list->NbrEntries <
	    (OS_OBJ_QTY)2) { /* See if it's time to time slice current task            */
		CPU_CRITICAL_EXIT(); /* ... only if multiple tasks at same priority            */
		return;
	}

	if (OSSchedLockNestingCtr >
	    (OS_NESTING_CTR)0) { /* Can't round-robin if the scheduler is locked           */
		CPU_CRITICAL_EXIT();
		return;
	}
...
void OS_TaskBlock(OS_TCB *p_tcb, OS_TICK timeout)
{
	...
	if (timeout >
	    (OS_TICK)0) { /* Add task to tick list if timeout non zero               */
...
{
	OS_TickListRemove(
		p_tcb); /* Remove from tick list                                  */
	if ((p_tcb->TaskState & OS_TASK_STATE_BIT_SUSPENDED) == (OS_STATE)0) {
		OS_RdyListInsert(
			p_tcb); /* Insert the task in the ready list                      */
	}
}
