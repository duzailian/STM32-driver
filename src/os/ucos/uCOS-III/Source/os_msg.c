...
OS_MSG_QTY OS_MsgQFreeAll(OS_MSG_Q *p_msg_q)
{
	...
	if (p_msg_q->NbrEntries > (OS_MSG_QTY)0) {
		...
		OSMsgPool.NextPtr =
			p_msg_q->OutPtr; /* Point to beginning of message chain                    */
		OSMsgPool.NbrUsed -=
			p_msg_q->NbrEntries; /* Update statistics for free list of messages            */
		OSMsgPool.NbrFree += p_msg_q->NbrEntries;
		p_msg_q->NbrEntries =
			(OS_MSG_QTY)0; /* Flush the message queue                                */
		 ...
		 OS_ERR *p_err)
{
	...
	if (p_msg_q->NbrEntries ==
	    (OS_MSG_QTY)0) { /* Is the queue empty?                                    */
		*p_msg_size =
			(OS_MSG_SIZE)0; /* Yes                                                    */
		if (p_ts != (CPU_TS *)0) {
			*p_ts = (CPU_TS)0;
	...
	*p_msg_size = p_msg->MsgSize;
	...
	p_msg_q->OutPtr =
		p_msg->NextPtr; /* Point to next message to extract                       */

	if (p_msg_q->OutPtr ==
	    (OS_MSG *)0) { /* Are there any more messages in the queue?              */
		p_msg_q->InPtr =
			(OS_MSG *)0; /* No                                                     */
		p_msg_q->NbrEntries = (OS_MSG_QTY)0;
	} else {
	...
	p_msg->NextPtr =
		OSMsgPool.NextPtr; /* Return message control block to free list              */
		...
		OS_OPT opt, CPU_TS ts, OS_ERR *p_err)
{
	...
	if (p_msg_q->NbrEntries >= p_msg_q->NbrEntriesSize) {
		*p_err =
			OS_ERR_Q_MAX; /* Message queue cannot accept any more messages          */
		return;
	}

	if (OSMsgPool.NbrFree == (OS_MSG_QTY)0) {
		*p_err =
			OS_ERR_MSG_POOL_EMPTY; /* No more OS_MSG to use                                  */
		return;
	}
	...
	if (p_msg_q->NbrEntries ==
	    (OS_MSG_QTY)0) { /* Is this first message placed in the queue?             */
		p_msg_q->InPtr =
			p_msg; /* Yes                                                    */
		...
		if ((opt & OS_OPT_POST_LIFO) ==
		    OS_OPT_POST_FIFO) { /* Is it FIFO or LIFO?                                    */
			p_msg_in =
				p_msg_q->InPtr; /* FIFO, add to the head                                  */
			p_msg_in->NextPtr = p_msg;
			p_msg_q->InPtr = p_msg;
			p_msg->NextPtr = (OS_MSG *)0;
		} else {
			p_msg->NextPtr =
				p_msg_q->OutPtr; /* LIFO, add to the tail                                  */
			p_msg_q->OutPtr = p_msg;
		}
		p_msg_q->NbrEntries++;
	}
	...
	p_msg->MsgPtr =
		p_void; /* Deposit message in the message queue entry             */
