...
#if (((OS_CFG_Q_EN > 0u) || (OS_CFG_SEM_EN > 0u)) &&                           \
     (OS_CFG_PEND_MULTI_EN > 0u))
/*
************************************************************************************************************************
*                                               PEND ON MULTIPLE OBJECTS
*
		       ...
		       OS_TICK timeout, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (p_pend_data_tbl ==
	    (OS_PEND_DATA
		     *)0) { /* Validate 'p_pend_data_tbl'                             */
		*p_err = OS_ERR_PTR_INVALID;
		return ((OS_OBJ_QTY)0);
	}
	if (tbl_size ==
	    (OS_OBJ_QTY)0) { /* Array size must be > 0                                 */
		*p_err = OS_ERR_PTR_INVALID;
		return ((OS_OBJ_QTY)0);
	}
	switch (opt) {
	...
	valid = OS_PendMultiValidate(
		p_pend_data_tbl, /* -------- Validate objects to be OS_SEM or OS_Q ------- */
		tbl_size);
	if (valid == DEF_FALSE) {
		*p_err =
			OS_ERR_OBJ_TYPE; /* Invalid, not OS_SEM or OS_Q                            */
		return ((OS_OBJ_QTY)0);
	}

	/*$PAGE*/
	CPU_CRITICAL_ENTER();
	nbr_obj_rdy = OS_PendMultiGetRdy(
		p_pend_data_tbl, /* --------- SEE IF OBJECT(s) HAVE BEEN POSTED ---------- */
		tbl_size);
	...
	OS_PendMultiWait(
		p_pend_data_tbl, /* Suspend task until object posted or timeout occurs     */
		tbl_size, timeout);
...
#if OS_CFG_Q_EN > 0u
		p_q = (OS_Q *)((void *)p_pend_data_tbl
				       ->PendObjPtr); /* Assume we are pointing to a message queue object  */
		if (p_q->Type ==
		    OS_OBJ_TYPE_Q) { /* Is it a message queue?                            */
			p_void = OS_MsgQGet(
				&p_q->MsgQ, /* Yes, Any message waiting in the message queue?    */
				&msg_size, &ts, &err);
			if (err == OS_ERR_NONE) {
				p_pend_data_tbl->RdyObjPtr =
					p_pend_data_tbl->PendObjPtr;
				p_pend_data_tbl->RdyMsgPtr =
					p_void; /*      Yes, save the message received               */
...
#if OS_CFG_SEM_EN > 0u
		p_sem = (OS_SEM *)((void *)p_pend_data_tbl
					   ->PendObjPtr); /* Assume we are pointing to a semaphore object      */
		if (p_sem->Type ==
		    OS_OBJ_TYPE_SEM) { /* Is it a semaphore?                                */
			if (p_sem->Ctr >
			    0u) { /* Yes, Semaphore has been signaled?                 */
				 ...
				 OS_OBJ_QTY tbl_size)
{
	...
	for (i = 0u; i < tbl_size; i++) {
		if (p_pend_data_tbl->PendObjPtr ==
		    (OS_PEND_OBJ
			     *)0) { /* All .PendObjPtr in the table MUST be non NULL     */
			return (DEF_FALSE);
		}
...
#if OS_CFG_SEM_EN > 0u
		p_sem = (OS_SEM *)((void *)p_pend_data_tbl
					   ->PendObjPtr); /* All objects to pend on must be of type OS_SEM ... */
		      ...
		      OS_TICK timeout)
{
	...
	OSTCBCurPtr->PendOn =
		OS_TASK_PEND_ON_MULTI; /* Resource not available, wait until it is            */
	OSTCBCurPtr->PendStatus = OS_STATUS_PEND_OK;
	OSTCBCurPtr->PendDataTblEntries = tbl_size;
	OSTCBCurPtr->PendDataTblPtr = p_pend_data_tbl;

	OS_TaskBlock(
		OSTCBCurPtr, /* Block the task waiting for object to be posted ...  */
		timeout); /* ... but with a timeout if not                       */

	for (i = 0u; i < tbl_size; i++) {
		p_pend_data_tbl->TCBPtr =
			OSTCBCurPtr; /* Every entry points back to the TCB of the task      */

#if OS_CFG_SEM_EN > 0u
		p_sem = (OS_SEM *)((void *)p_pend_data_tbl->PendObjPtr);
		if (p_sem->Type == OS_OBJ_TYPE_SEM) {
			p_pend_list = &p_sem->PendList;
			OS_PendListInsertPrio(p_pend_list, p_pend_data_tbl);
		}
#endif
