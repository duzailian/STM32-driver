...

{
	...
	if (p_q ==
	    (OS_Q *)0) { /* Validate arguments                                     */
		*p_err = OS_ERR_OBJ_PTR_NULL;
		return;
	}
	if (max_qty ==
	    (OS_MSG_QTY)0) { /* Cannot specify a zero size queue                       */
		*p_err = OS_ERR_Q_SIZE;
		return;
	}
#endif
	...
	OS_CRITICAL_ENTER();
	p_q->Type =
		OS_OBJ_TYPE_Q; /* Mark the data structure as a message queue             */
	p_q->NamePtr = p_name;
	OS_MsgQInit(
		&p_q->MsgQ, /* Initialize the queue                                   */
		max_qty);
	OS_PendListInit(
...
OS_OBJ_QTY OSQDel(OS_Q *p_q, OS_OPT opt, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Can't delete a message queue from an ISR               */
	...
	if (p_q->Type !=
	    OS_OBJ_TYPE_Q) { /* Make sure message queue was created                    */
	...
	case OS_OPT_DEL_ALWAYS: /* Always delete the message queue                        */
		...
		while (cnt >
			...
			OS_PendObjDel((OS_PEND_OBJ *)((void *)p_q), p_tcb, ts);
			cnt--;
		}
#if OS_CFG_DBG_EN > 0u
		OS_QDbgListRemove(p_q);
...
OS_MSG_QTY OSQFlush(OS_Q *p_q, OS_ERR *p_err)
{
	...
	entries = OS_MsgQFreeAll(
		&p_q->MsgQ); /* Return all OS_MSGs to the OS_MSG pool                  */
	OS_CRITICAL_EXIT();
	*p_err = OS_ERR_NONE;
	      ...
	      CPU_TS *p_ts, OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	p_void = OS_MsgQGet(
		&p_q->MsgQ, /* Any message waiting in the message queue?              */
		p_msg_size, p_ts, p_err);
	...
	OS_Pend(&pend_data, /* Block task pending on Message Queue                    */
		(OS_PEND_OBJ *)((void *)p_q), OS_TASK_PEND_ON_Q, timeout);
	OS_CRITICAL_EXIT_NO_SCHED();
...
OS_OBJ_QTY OSQPendAbort(OS_Q *p_q, OS_OPT opt, OS_ERR *p_err)
{
...
#if OS_CFG_OBJ_TYPE_CHK_EN > 0u
	if (p_q->Type !=
	    OS_OBJ_TYPE_Q) { /* Make sure queue was created                            */
		*p_err = OS_ERR_OBJ_TYPE;
		return ((OS_OBJ_QTY)0u);
	}
#endif
	...
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0u) { /* Any task waiting on queue?                             */
	...
	while (p_pend_list->NbrEntries > (OS_OBJ_QTY)0u) {
		...
		OS_PendAbort((OS_PEND_OBJ *)((void *)p_q), p_tcb, ts);
		nbr_tasks++;
		if (opt !=
	     ...
	     OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr > (OS_NESTING_CTR)0) {
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_Q, /* Post to ISR queue                                      */
			(void *)p_q, (void *)p_void, (OS_MSG_SIZE)msg_size,
			(OS_FLAGS)0, (OS_OPT)opt, (CPU_TS)ts, (OS_ERR *)p_err);
		return;
	}
#endif

	OS_QPost(p_q, p_void, msg_size, opt, ts, p_err);
}
...
{
	(void)OS_MsgQFreeAll(
		&p_q->MsgQ); /* Return all OS_MSGs to the free list                    */
	p_q->Type =
		OS_OBJ_TYPE_NONE; /* Mark the data structure as a NONE                      */
	p_q->NamePtr = (CPU_CHAR *)((void *)"?Q");
	OS_MsgQInit(
		&p_q->MsgQ, /* Initialize the list of OS_MSGs                         */
		0u);
	OS_PendListInit(
		&p_q->PendList); /* Initialize the waiting list                            */
}
	      ...
	      CPU_TS ts, OS_ERR *p_err)
{
	...
	p_pend_list = &p_q->PendList;
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0) { /* Any task waiting on message queue?                     */
		if ((opt & OS_OPT_POST_LIFO) ==
		    (OS_OPT)0) { /* Determine whether we post FIFO or LIFO                 */
			post_type = OS_OPT_POST_FIFO;
		} else {
			post_type = OS_OPT_POST_LIFO;
		}
		OS_MsgQPut(
			&p_q->MsgQ, /* Place message in the message queue                     */
			p_void, msg_size, post_type, ts, p_err);
		OS_CRITICAL_EXIT();
		return;
	}

	if ((opt & OS_OPT_POST_ALL) !=
	    (OS_OPT)0) { /* Post message to all tasks waiting?                     */
		cnt = p_pend_list
			      ->NbrEntries; /* Yes                                                    */
	} else {
		cnt = (OS_OBJ_QTY)1; /* No                                                     */
	...
	while (cnt > 0u) {
		...
		OS_Post((OS_PEND_OBJ *)((void *)p_q), p_tcb, p_void, msg_size,
			ts);
