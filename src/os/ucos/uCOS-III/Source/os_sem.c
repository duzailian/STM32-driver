...
void OSSemCreate(OS_SEM *p_sem, CPU_CHAR *p_name, OS_SEM_CTR cnt, OS_ERR *p_err)
{
	...
	OS_CRITICAL_ENTER();
	p_sem->Type =
		OS_OBJ_TYPE_SEM; /* Mark the data structure as a semaphore                 */
	p_sem->Ctr =
		cnt; /* Set semaphore value                                    */
	p_sem->TS = (CPU_TS)0;
	p_sem->NamePtr =
		p_name; /* Save the name of the semaphore                         */
	OS_PendListInit(
...
OS_OBJ_QTY OSSemDel(OS_SEM *p_sem, OS_OPT opt, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Not allowed to delete a semaphore from an ISR          */
	...
	if (p_sem->Type !=
	    OS_OBJ_TYPE_SEM) { /* Make sure semaphore was created                        */
	...
	case OS_OPT_DEL_ALWAYS: /* Always delete the semaphore                            */
		...
		while (cnt >
		       0u) { /* Remove all tasks on the pend list                      */
			p_pend_data = p_pend_list->HeadPtr;
			p_tcb = p_pend_data->TCBPtr;
			OS_PendObjDel((OS_PEND_OBJ *)((void *)p_sem), p_tcb,
				      ts);
			cnt--;
		}
#if OS_CFG_DBG_EN > 0u
		     ...
		     OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	if (p_sem->Ctr >
	    (OS_SEM_CTR)0) { /* Resource available?                                    */
	...
	OS_Pend(&pend_data, /* Block task pending on Semaphore                        */
		(OS_PEND_OBJ *)((void *)p_sem), OS_TASK_PEND_ON_SEM, timeout);
...
OS_OBJ_QTY OSSemPendAbort(OS_SEM *p_sem, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0u) { /* Any task waiting on semaphore?                         */
	...
	while (p_pend_list->NbrEntries > (OS_OBJ_QTY)0u) {
		...
		OS_PendAbort((OS_PEND_OBJ *)((void *)p_sem), p_tcb, ts);
		nbr_tasks++;
		if (opt !=
...
OS_SEM_CTR OSSemPost(OS_SEM *p_sem, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
		...
		OS_IntQPost(
			(OS_OBJ_TYPE)
				OS_OBJ_TYPE_SEM, /* Post to ISR queue                                      */
			(void *)p_sem, (void *)0, (OS_MSG_SIZE)0, (OS_FLAGS)0,
...
void OSSemSet(OS_SEM *p_sem, OS_SEM_CTR cnt, OS_ERR *p_err)
{
	...
	*p_err = OS_ERR_NONE;
	CPU_CRITICAL_ENTER();
	if (p_sem->Ctr >
	    (OS_SEM_CTR)0) { /* See if semaphore already has a count                   */
		p_sem->Ctr =
			cnt; /* Yes, set it to the new value specified.                */
	} else {
		p_pend_list =
			&p_sem->PendList; /* No                                                     */
		if (p_pend_list->NbrEntries ==
		    (OS_OBJ_QTY)0) { /*      See if task(s) waiting?                           */
			p_sem->Ctr =
				cnt; /*      No, OK to set the value                           */
		} else {
			*p_err = OS_ERR_TASK_WAITING;
		}
	}
...
void OS_SemClr(OS_SEM *p_sem)
{
	...
	p_sem->Ctr =
		(OS_SEM_CTR)0; /* Set semaphore value                                    */
	p_sem->TS =
		(CPU_TS)0; /* Clear the time stamp                                   */
...
OS_SEM_CTR OS_SemPost(OS_SEM *p_sem, OS_OPT opt, CPU_TS ts, OS_ERR *p_err)
{
	...
	p_pend_list = &p_sem->PendList;
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0) { /* Any task waiting on semaphore?                         */
		...
		ctr = p_sem->Ctr;
		p_sem->TS =
			ts; /* Save timestamp in semaphore control block              */
		CPU_CRITICAL_EXIT();
		*p_err = OS_ERR_NONE;
		return (ctr);
	}
	...
	while (cnt > 0u) {
		...
		OS_Post((OS_PEND_OBJ *)((void *)p_sem), p_tcb, (void *)0,
			(OS_MSG_SIZE)0, ts);
