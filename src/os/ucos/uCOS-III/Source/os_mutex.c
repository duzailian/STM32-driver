...
void OSMutexCreate(OS_MUTEX *p_mutex, CPU_CHAR *p_name, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Not allowed to be called from an ISR                   */
	...
	OS_CRITICAL_ENTER();
	p_mutex->Type =
		OS_OBJ_TYPE_MUTEX; /* Mark the data structure as a mutex                     */
	p_mutex->NamePtr = p_name;
	p_mutex->OwnerTCBPtr = (OS_TCB *)0;
	p_mutex->OwnerNestingCtr =
		(OS_NESTING_CTR)0; /* Mutex is available                                     */
	p_mutex->TS = (CPU_TS)0;
...
OS_OBJ_QTY OSMutexDel(OS_MUTEX *p_mutex, OS_OPT opt, OS_ERR *p_err)
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* Not allowed to delete a mutex from an ISR          */
	...
	if (p_mutex->Type !=
	    OS_OBJ_TYPE_MUTEX) { /* Make sure mutex was created                        */
	...
	case OS_OPT_DEL_ALWAYS: /* Always delete the mutex                 */
		p_tcb_owner =
			p_mutex->OwnerTCBPtr; /* Did we had to change the prio of owner? */
		if ((p_tcb_owner != (OS_TCB *)0) &&
			...
			case OS_TASK_STATE_RDY:
				...
				p_tcb_owner->Prio =
					p_mutex->OwnerOriginalPrio; /* Lower owner's prio back                 */
				OS_PrioInsert(p_tcb_owner->Prio);
				OS_RdyListInsertTail(
					p_tcb_owner); /* Insert owner in ready list at new prio  */
				break;
			...
			case OS_TASK_STATE_DLY_SUSPENDED:
				p_tcb_owner->Prio =
					p_mutex->OwnerOriginalPrio; /* Not in any pend list, change the prio   */
				break;
			...
			case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
				OS_PendListChangePrio(
					p_tcb_owner, /* Owner is pending on another object      */
		...
		while (cnt > 0u) { /* Remove all tasks from the pend list     */
			...
			OS_PendObjDel((OS_PEND_OBJ *)((void *)p_mutex), p_tcb,
				      ts);
			cnt--;
		}
#if OS_CFG_DBG_EN > 0u
		 ...
		 OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	if (p_mutex->OwnerNestingCtr ==
	    (OS_NESTING_CTR)0) { /* Resource available?                                    */
		p_mutex->OwnerTCBPtr =
			OSTCBCurPtr; /* Yes, caller may proceed                                */
	...
	if (OSTCBCurPtr ==
	    p_mutex->OwnerTCBPtr) { /* See if current task is already the owner of the mutex  */
		p_mutex->OwnerNestingCtr++;
		if (p_ts != (CPU_TS *)0) {
			*p_ts = p_mutex->TS;
		}
		CPU_CRITICAL_EXIT();
		*p_err =
			OS_ERR_MUTEX_OWNER; /* Indicate that current task already owns the mutex      */
		return;
	}
	...
	if (p_tcb->Prio >
	    OSTCBCurPtr
		    ->Prio) { /* See if mutex owner has a lower priority than current   */
		switch (p_tcb->TaskState) {
		case OS_TASK_STATE_RDY:
			OS_RdyListRemove(
				p_tcb); /* Remove from ready list at current priority             */
			p_tcb->Prio =
				OSTCBCurPtr
					->Prio; /* Raise owner's priority                                 */
			OS_PrioInsert(p_tcb->Prio);
			OS_RdyListInsertHead(
				p_tcb); /* Insert in ready list at new priority                   */
			break;
		...
		case OS_TASK_STATE_SUSPENDED:
			p_tcb->Prio =
				OSTCBCurPtr
					->Prio; /* Only need to raise the owner's priority                */
			break;
		...
		case OS_TASK_STATE_PEND_SUSPENDED:
		case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
			OS_PendListChangePrio(p_tcb, OSTCBCurPtr->Prio);
			break;
	...
	OS_Pend(&pend_data, /* Block task pending on Mutex                            */
		(OS_PEND_OBJ *)((void *)p_mutex), OS_TASK_PEND_ON_MUTEX,
		timeout);
...
OS_OBJ_QTY OSMutexPendAbort(OS_MUTEX *p_mutex, OS_OPT opt, OS_ERR *p_err)
{
	...
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0u) { /* Any task waiting on mutex?                             */
	...
	while (p_pend_list->NbrEntries > (OS_OBJ_QTY)0u) {
		...
		OS_PendAbort((OS_PEND_OBJ *)((void *)p_mutex), p_tcb, ts);
		nbr_tasks++;
		if (opt !=
...
void OSMutexPost(OS_MUTEX *p_mutex, OS_OPT opt, OS_ERR *p_err)
{
	...
	CPU_CRITICAL_ENTER();
	if (OSTCBCurPtr !=
	    p_mutex->OwnerTCBPtr) { /* Make sure the mutex owner is releasing the mutex       */
		CPU_CRITICAL_EXIT();
	...
	if (p_mutex->OwnerNestingCtr >
	    (OS_NESTING_CTR)0) { /* Are we done with all nestings?                         */
		OS_CRITICAL_EXIT(); /* No                                                     */
	...
	p_pend_list = &p_mutex->PendList;
	if (p_pend_list->NbrEntries ==
	    (OS_OBJ_QTY)0) { /* Any task waiting on mutex?                             */
		p_mutex->OwnerTCBPtr =
			(OS_TCB *)0; /* No                                                     */
	...
	if (OSTCBCurPtr->Prio != p_mutex->OwnerOriginalPrio) {
		...
		OSTCBCurPtr->Prio =
			p_mutex->OwnerOriginalPrio; /* Lower owner's priority back to its original one        */
		OS_PrioInsert(OSTCBCurPtr->Prio);
		OS_RdyListInsertTail(
			OSTCBCurPtr); /* Insert owner in ready list at new priority             */
		OSPrioCur = OSTCBCurPtr->Prio;
	}
	/* Get TCB from head of pend list                         */
	p_tcb = p_pend_list->HeadPtr->TCBPtr;
	p_mutex->OwnerTCBPtr =
		p_tcb; /* Give mutex to new owner                                */
	p_mutex->OwnerOriginalPrio = p_tcb->Prio;
	p_mutex->OwnerNestingCtr = (OS_NESTING_CTR)1;
	/* Post to mutex                                          */
	OS_Post((OS_PEND_OBJ *)((void *)p_mutex), (OS_TCB *)p_tcb, (void *)0,
		(OS_MSG_SIZE)0, (CPU_TS)ts);
...
{
	p_mutex->Type =
		OS_OBJ_TYPE_NONE; /* Mark the data structure as a NONE                      */
