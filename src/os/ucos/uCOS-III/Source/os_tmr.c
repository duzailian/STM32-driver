		 ...
		 void *p_callback_arg, OS_ERR *p_err)
{
	...
	if (OSIntNestingCtr >
	    (OS_NESTING_CTR)0) { /* See if trying to call from an ISR                      */
	...
	OS_CRITICAL_ENTER();
	p_tmr->State = (OS_STATE)
		OS_TMR_STATE_STOPPED; /* Initialize the timer fields             */
...
CPU_BOOLEAN OSTmrDel(OS_TMR *p_tmr, OS_ERR *p_err)
{
	...
	if (p_tmr->Type !=
	    OS_OBJ_TYPE_TMR) { /* Make sure timer was created                            */
	...
	case OS_TMR_STATE_RUNNING:
		OS_TmrUnlink(
			p_tmr); /* Remove from current wheel spoke                        */
		OS_TmrClr(p_tmr);
		OSSchedUnlock(&err);
		*p_err = OS_ERR_NONE;
		success = DEF_TRUE;
...
OS_TICK OSTmrRemainGet(OS_TMR *p_tmr, OS_ERR *p_err)
{
	...
	case OS_TMR_STATE_RUNNING:
		remain =
			p_tmr->Match /* Determine how much time is left to timeout             */
			- OSTmrTickCtr;
		p_tmr->Remain = remain;
		OSSchedUnlock(&err);
		*p_err = OS_ERR_NONE;
...
CPU_BOOLEAN OSTmrStart(OS_TMR *p_tmr, OS_ERR *p_err)
{
	...
	case OS_TMR_STATE_RUNNING: /* Restart the timer                                      */
		...
		OS_TmrLink(
			p_tmr,
			OS_OPT_LINK_DLY); /* ... Link timer to timer wheel (see Note #1).           */
		OSSchedUnlock(&err);
		*p_err = OS_ERR_NONE;
		success = DEF_TRUE;
		break;
	...
	case OS_TMR_STATE_RUNNING:
		...
		*p_err = OS_ERR_NONE;
		...
		case OS_OPT_TMR_CALLBACK:
			p_fnct =
				p_tmr->CallbackPtr; /* Execute callback function ...           */
			if (p_fnct !=
			    (OS_TMR_CALLBACK_PTR)0) { /* ... if available                        */
				(*p_fnct)(
					(void *)p_tmr,
					p_tmr->CallbackPtrArg); /* Use callback arg when timer was created */
			} else {
		...
		case OS_OPT_TMR_CALLBACK_ARG:
			p_fnct =
				p_tmr->CallbackPtr; /* Execute callback function if available ...   */
			if (p_fnct != (OS_TMR_CALLBACK_PTR)0) {
				(*p_fnct)(
					(void *)p_tmr,
					p_callback_arg); /* .. using the 'callback_arg' provided in call */
			} else {
				*p_err = OS_ERR_TMR_NO_CALLBACK;
			}
			break;
...
{
	p_tmr->State =
		OS_TMR_STATE_UNUSED; /* Clear timer fields                                     */
...
void OS_TmrLink(OS_TMR *p_tmr, OS_OPT opt)
{
	...
	if (opt ==
	    OS_OPT_LINK_PERIODIC) { /* Determine when timer will expire                      */
	...
	if (p_spoke->FirstPtr ==
	    (OS_TMR *)0) { /* Link into timer wheel                                  */
		p_tmr->NextPtr = (OS_TMR *)0;
		p_tmr->PrevPtr = (OS_TMR *)0;
		...
		p_tmr->Remain =
			p_tmr->Match /* Compute remaining time for timer                       */
			- OSTmrTickCtr;
		p_tmr1 =
			p_spoke->FirstPtr; /* Point to current first timer in the list               */
		while (p_tmr1 != (OS_TMR *)0) {
			p_tmr1->Remain =
				p_tmr1->Match /* Compute time remaining of current timer in list        */
				- OSTmrTickCtr;
			if (p_tmr->Remain >
			    p_tmr1->Remain) { /* Do we need to insert AFTER current timer in list?      */
				if (p_tmr1->NextPtr !=
				    (OS_TMR *)0) { /* Yes, are we pointing at the last timer in the list?    */
					p_tmr1 =
						p_tmr1->NextPtr; /* No,  Point to next timer in the list                   */
				} else {
					p_tmr->NextPtr = (OS_TMR *)0;
					p_tmr->PrevPtr = p_tmr1;
					p_tmr1->NextPtr =
						p_tmr; /* Yes, timer to insert is now new last entry in the list */
					p_tmr1 =
						(OS_TMR *)0; /* Break loop                                             */
				}
			} else { /* Insert before the current timer                        */
				if (p_tmr1->PrevPtr ==
				    (OS_TMR *)0) { /* Are we inserting before the first timer?               */
					p_tmr->PrevPtr = (OS_TMR *)0;
	...
	if (p_spoke->NbrEntriesMax <
	    p_spoke->NbrEntries) { /* Keep track of maximum number of entries in each spoke  */
...
void OS_TmrUnlink(OS_TMR *p_tmr)
{
	...
	if (p_spoke->FirstPtr ==
	    p_tmr) { /* See if timer to remove is at the beginning of list     */
		...
		p_tmr1 =
			(OS_TMR *)p_tmr
				->PrevPtr; /* Remove timer from somewhere in the list                */
...
void OS_TmrTask(void *p_arg)
{
	...
	while (DEF_ON) {
		(void)OSTaskSemPend(
			(OS_TICK)0, /* Wait for signal indicating time to update tmrs    */
		...
		while (done == DEF_FALSE) {
			if (p_tmr != (OS_TMR *)0) {
				p_tmr_next =
					(OS_TMR *)p_tmr
						->NextPtr; /* Point to next tmr to update because current ...   */
				/* ... timer could get unlinked from the wheel.      */
				if (OSTmrTickCtr ==
				    p_tmr->Match) { /* Process each timer that expires                   */
					OS_TmrUnlink(
						p_tmr); /* Remove from current wheel spoke                   */
					if (p_tmr->Opt == OS_OPT_TMR_PERIODIC) {
						OS_TmrLink(
						...
						p_tmr->State =
							OS_TMR_STATE_COMPLETED; /* Indicate that the timer has completed             */
					}
					p_fnct =
						p_tmr->CallbackPtr; /* Execute callback function if available            */
					if (p_fnct != (OS_TMR_CALLBACK_PTR)0) {
		...
		ts_end =
			OS_TS_GET() -
			ts_start; /* Measure execution time of timer task              */
		OSSchedUnlock(&err);
