...
CPU_INT08U const OSDbg_DbgEn =
	OS_CFG_DBG_EN; /* Debug constants are defined below   */
...
CPU_INT32U const OSDbg_EndiannessTest =
	0x12345678LU; /* Variable to test CPU endianness     */
...
CPU_INT16U const OSDbg_FlagGrpSize =
	sizeof(OS_FLAG_GRP); /* Size in Bytes of OS_FLAG_GRP        */
CPU_INT16U const OSDbg_FlagWidth =
	sizeof(OS_FLAGS); /* Width (in bytes) of OS_FLAGS        */
#else
...
#if OS_CFG_MEM_EN > 0u
CPU_INT16U const OSDbg_MemSize =
	sizeof(OS_MEM); /* Mem. Partition header size (bytes)  */
#else
...
CPU_INT16U const OSDbg_MsgSize =
	sizeof(OS_MSG); /* OS_MSG size                         */
...
CPU_INT16U const OSDbg_MutexSize =
	sizeof(OS_MUTEX); /* Size in bytes of OS_MUTEX           */
#else
...
CPU_INT16U const OSDbg_PrioMax =
	OS_CFG_PRIO_MAX; /* Maximum number of priorities        */
CPU_INT16U const OSDbg_PrioTblSize = sizeof(OSPrioTbl);

CPU_INT16U const OSDbg_PtrSize =
	sizeof(void *); /* Size in Bytes of a pointer          */
...
CPU_INT16U const OSDbg_QSize =
	sizeof(OS_Q); /* Size in bytes of OS_Q structure     */
#else
...
CPU_INT16U const OSDbg_SemSize =
	sizeof(OS_SEM); /* Size in bytes of OS_SEM             */
#else
...
CPU_INT32U const OSDbg_RdyListSize =
	sizeof(OSRdyList); /* Number of bytes in the ready table  */
...
CPU_INT16U const OSDbg_TCBSize =
	sizeof(OS_TCB); /* Size in Bytes of OS_TCB             */
...
CPU_INT32U const OSDbg_DataSize =
	sizeof(OSIntNestingCtr)

#if OS_CFG_APP_HOOKS_EN > 0u
	+ sizeof(OS_AppTaskCreateHookPtr) + sizeof(OS_AppTaskDelHookPtr) +
	sizeof(OS_AppTaskReturnHookPtr)

	+ sizeof(OS_AppIdleTaskHookPtr) + sizeof(OS_AppStatTaskHookPtr) +
	sizeof(OS_AppTaskSwHookPtr) + sizeof(OS_AppTimeTickHookPtr)
#endif

	+ sizeof(OSIdleTaskCtr) + sizeof(OSIdleTaskTCB)
...
#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
	+ sizeof(OSIntQInPtr) + sizeof(OSIntQOutPtr) +
	sizeof(OSIntQNbrEntries) + sizeof(OSIntQNbrEntriesMax) +
	sizeof(OSIntQOvfCtr) + sizeof(OSIntQTaskTCB) + sizeof(OSIntQTaskTimeMax)
#endif
...
#if OS_CFG_FLAG_EN > 0u
	+ sizeof(OSFlagDbgListPtr) + sizeof(OSFlagQty)
#endif
	...
	+ sizeof(OSPrioCur) + sizeof(OSPrioHighRdy) + sizeof(OSPrioSaved) +
	sizeof(OSPrioTbl)
	...
	+ sizeof(OSSchedLockTimeBegin) + sizeof(OSSchedLockTimeMax) +
	sizeof(OSSchedLockTimeMaxCur)
#endif

#if OS_CFG_SCHED_ROUND_ROBIN_EN
	+ sizeof(OSSchedRoundRobinDfltTimeQuanta) + sizeof(OSSchedRoundRobinEn)
#endif
	...
	+ sizeof(OSStatResetFlag) + sizeof(OSStatTaskCPUUsage) +
	sizeof(OSStatTaskCPUUsageMax) + sizeof(OSStatTaskCtr) +
	sizeof(OSStatTaskCtrMax) + sizeof(OSStatTaskCtrRun) +
	sizeof(OSStatTaskRdy) + sizeof(OSStatTaskTCB) +
	sizeof(OSStatTaskTimeMax)
#endif

	+ sizeof(OSTickCtr) + sizeof(OSTickTaskTCB) + sizeof(OSTickTaskTimeMax)

#if OS_CFG_TMR_EN > 0u
#if OS_CFG_DBG_EN > 0u
	+ sizeof(OSTmrDbgListPtr)
#endif
	+ sizeof(OSTmrQty) + sizeof(OSTmrTaskTCB) + sizeof(OSTmrTaskTimeMax) +
	sizeof(OSTmrTickCtr) + sizeof(OSTmrUpdateCnt) + sizeof(OSTmrUpdateCtr)
#endif
	...
	+ sizeof(OSTCBCurPtr) + sizeof(OSTCBHighRdyPtr);
...
void OS_Dbg_Init(void)
{
	...
	p_temp08 =
		p_temp08; /* Prevent compiler warning for not using 'p_temp'        */
	p_temp16 = p_temp16;
	p_temp32 = p_temp32;
}
#endif
